...
public:
    ...
    void process(const float* const* inputs, size_t numInputs, size_t startFrame, size_t numFrames)
    {
...
class TestStats
{
    ...
    void report(const World<TestStats>& world, TestStatsType type, float value)
    {
        switch (type) {
...
public:
    TestStats(const World<TestStats>& world, const Methcla_SynthDef*, const TestStatsOptions& options)
    ...
    void process(const World<TestStats>& world, size_t numFrames)
    {
        if (!m_done) {
            ...
            m_maxAbsAmp.process(m_inputs, TestStatsPorts::numPorts(), startFrame, activeNumFrames);
...
METHCLA_EXPORT const Methcla_Library* methcla_plugins_test_support(const Methcla_Host* host, const char* /* bundlePath */)
{
