...
namespace Catch
{
    ...
    struct CallbackReporter : StreamingReporterBase
    {
        ...
        virtual void sectionEnded( SectionStats const& _sectionStats )
        {
            ...
            if( m_headerPrinted )
            {
                if( m_config->showDurations() == ShowDurations::Always )
                    m_stream << "Completed in " << _sectionStats.durationInSeconds << "s" << std::endl;
                m_headerPrinted = false;
            }
            else
            {
                if( m_config->showDurations() == ShowDurations::Always )
                    m_stream << _sectionStats.sectionInfo.name << " completed in " << _sectionStats.durationInSeconds << "s" << std::endl;
            }
        ...
        virtual void testGroupEnded( TestGroupStats const& _testGroupStats )
        {
            if( currentGroupInfo.used ) {
                printSummaryDivider();
                m_stream << "Summary for group '" << _testGroupStats.groupInfo.name << "':\n";
    ...
    public:
        CallbackReporterRegistrar( std::string const& name, CallbackReporter::Callback callback )
        {
            getMutableRegistryHub().registerReporter( name, new ReporterFactory(callback) );
        }

    };

    inline static void registerCallbackReporter(const std::string& name, CallbackReporter::Callback callback)
    {
