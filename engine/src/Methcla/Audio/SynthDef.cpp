...
using namespace Methcla::Audio::Plugin;
using namespace std;
...
std::shared_ptr<Binary> StaticLoader::load(const LilvPlugin* plugin)
{
    ...
    if (uri == 0)
        return std::shared_ptr<Methcla::Audio::Plugin::Binary>();
...
void Manager::loadPlugins(const boost::filesystem::path& directory)
{
    ...
    cout << "Methcla::Audio::Plugin::Manager: " << lilv_plugins_size(plugins) << " plugins found" << endl;
