...
public:
    ...
    const ControlSpecVector& controlInputSpecs() const
    {
        return m_controlInputSpecs;
    }
    const ControlSpecVector& controlOutputSpecs() const
    {
        return m_controlOutputSpecs;
    }
    ...
    virtual void addVerticalSlider(const char*, float* zone, float init,
    ...
    virtual void addHorizontalSlider(const char*, float* zone, float init,
                                     float min, float max, float step)
    {
        Plugin::MetaData* md = addInputZone(zone);
        md->insert("min", min);
    ...
    virtual void addNumEntry(const char*, float* zone, float init, float min,
    ...
    virtual void addTextDisplay(const char*, float* zone, const char* names[],
                                float min, float max)
    {
        Plugin::MetaData* md = addOutputZone(zone);
        md->insert("min", min);
    ...
    virtual void addHorizontalBargraph(const char*, float* zone, float min,
                                       float max)
    {
        Plugin::MetaData* md = addOutputZone(zone);
        md->insert("min", min);
    ...
    virtual void addVerticalBargraph(const char*, float* zone, float min,
                                     float max)
    {
        Plugin::MetaData* md = addOutputZone(zone);
        md->insert("min", min);
...
public:
    ...
    virtual void addVerticalSlider(const char*, float* zone, float, float,
                                   float, float)
    {
        addControlInput(zone);
    }
    virtual void addHorizontalSlider(const char*, float* zone, float, float,
                                     float, float)
    {
        addControlInput(zone);
    }
    virtual void addNumEntry(const char*, float* zone, float, float, float,
                             float)
    {
        addControlInput(zone);
    }
    ...
    virtual void addTextDisplay(const char*, float* zone, const char**, float,
                                float)
    {
        addControlOutput(zone);
    }
...
{
public:
    MethclaFaustSynth(MethclaHost*                               host,
                      const Plugin::SynthDef<MethclaFaustSynth>* synthDef)
    : m_controlInputs(reinterpret_cast<float**>(reinterpret_cast<char*>(this) +
                                                sizeof(MethclaFaustSynth)))
...
{
public:
    SynthDef(const char* name, size_t numAudioInputs, size_t numAudioOutputs,
             const vector<Plugin::ControlSpec*>& controlInputs,
             const vector<Plugin::ControlSpec*>& controlOutputs)
    : Plugin::SynthDef<MethclaFaustSynth>(name, numAudioInputs, numAudioOutputs,
...
METHCLA_EXPORT void METHCLA_INIT_FUNC(FAUSTCLASS)(MethclaHost* host)
{
    ...
    try
    {
        ...
        ui.finish();
        MethclaSynthDef* def =
            new SynthDef(METHCLA_STRINGIFY(FAUSTCLASS), dsp->getNumInputs(),
                         dsp->getNumOutputs(), ui.controlInputSpecs(),
