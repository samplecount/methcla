...
struct SoundFileHandle
{
    ...
    {
        return methcla_error_new_with_message(code, mpg123_strerror(handle));
    }
};
                                    ...
                                    int64_t                  numFrames)
{
    ...
    off_t            n = mpg123_seek(handle->handle,
                          static_cast<off_t>(std::max<int64_t>(0, numFrames)),
                          SEEK_SET);
                                          ...
                                          size_t* outNumFrames)
{
    ...
    int err =
        mpg123_read(handle->handle, reinterpret_cast<unsigned char*>(buffer),
                    inNumFrames * handle->frameSize, &numBytes);
                                    ...
                                    Methcla_SoundFileInfo* info)
{
    ...
    if (mpg123_param(handle->handle, MPG123_ADD_FLAGS, MPG123_FORCE_FLOAT,
                     0.) != MPG123_OK)
        return handle->error();
    ...
    // Check number of frames in file here because mpg123_open doesn't return an
    // error for an invalid mp3 file
    ...
    if (mpg123_getformat(handle->handle, &rate, &channels, &encoding) !=
        MPG123_OK)
        return handle->error();
