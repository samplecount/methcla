...
namespace OSCPP { namespace Server {
    ...
    public:
        ...
        size_t size() const { return m_tags.capacity(); }

        //* Return true if no more arguments can be read from the stream.
        bool atEnd() const { return m_tags.atEnd(); }
        ...
        char tag() const { return m_tags.peekChar(); }

        //* Drop next argument.
        void drop() { drop(m_tags.getChar()); }
         ...
         *
         * \exception OSCPP::UnderrunError stream buffer underrun.
         * \exception OSCPP::ParseError argument could not be converted or is
         * not a valid string.
         */
        const char* string()
        {
    ...
    public:
        ...
        ArgStream args() const { return m_args; }
    ...
    public:
        ...
        const void* data() const { return m_stream.begin(); }

        size_t size() const { return m_stream.capacity(); }

        bool isBundle() const { return m_isBundle; }
    ...
    template <> inline int32_t ArgStream::next<int32_t>() { return int32(); }

    template <> inline float ArgStream::next<float>() { return float32(); }
    ...
    template <> inline Blob ArgStream::next<Blob>() { return blob(); }
    ...
    PacketStream Bundle::packets() const { return PacketStream(m_stream); }
