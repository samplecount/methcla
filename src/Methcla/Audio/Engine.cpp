...
namespace {
    ...
    METHCLA_C_LINKAGE Methcla_Error methcla_api_host_soundfile_open(
        const Methcla_Host* host, const char* path, Methcla_FileMode mode,
        Methcla_SoundFile** file, Methcla_SoundFileInfo* info)
    {
        ...
        // Open sound file with first API that matches the file extension or
        // doesn't return an error (if the API doesn't specify file extensions).
        for (auto it = apis.begin(); it != apis.end(); it++)
        {
                               ...
                               takeExtension(path)))
            {
                ...
                else if (!(methcla_error_has_code(
                               result, kMethcla_UnsupportedFileTypeError) ||
                           methcla_error_has_code(
    ...
    METHCLA_C_LINKAGE double
    methcla_api_world_samplerate(const Methcla_World* world)
    {
...
void Environment::process(Methcla_Time currentTime, size_t numFrames,
...
void Environment::reply(Methcla_RequestId            requestId,
    ...
    Methcla_Host* host, Methcla_WorldPerformFunction perform, void* data)
{
    ...
    CallbackData<Methcla_WorldPerformFunction>* callbackData =
        Methcla::Memory::allocOf<CallbackData<Methcla_WorldPerformFunction>>();
    ...
    Methcla_World* world, Methcla_HostPerformFunction perform, void* data)
{
    ...
    CallbackData<Methcla_HostPerformFunction>* callbackData =
        env->rtMem().allocOf<CallbackData<Methcla_HostPerformFunction>>();
    callbackData->func = perform;
    callbackData->arg = data;
