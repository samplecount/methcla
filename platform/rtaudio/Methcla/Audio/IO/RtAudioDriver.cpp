...
int RtAudioDriver::processCallback(void* outputBuffer, void* inputBuffer,
                                   ...
                                   RtAudioStreamStatus, void*     data)
{
    static_cast<RtAudioDriver*>(data)->process(
        static_cast<float*>(outputBuffer), static_cast<float*>(inputBuffer),
        numFrames, streamTime);
    return 0;
}

void RtAudioDriver::process(float* outputBuffer, const float* inputBuffer,
                            unsigned int numFrames, double streamTime)
{
    ...
    Driver::process(streamTime, m_outputBuffer->numFrames(),
...
Methcla_AudioDriver*
methcla_platform_rtaudio_driver_new(const Methcla_AudioDriverOptions* options)
{
    return Methcla::API::wrapAudioDriver(new Methcla::Audio::IO::RtAudioDriver(
