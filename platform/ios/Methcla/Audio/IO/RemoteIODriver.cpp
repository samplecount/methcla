...
#define METHCLA_CASE_SYSTEM_ERROR(status) \
    case status:                          \
        throw Error(kMethcla_SystemError, #status)
...
static void initStreamFormat(AudioStreamBasicDescription& desc,
                             Float64 sampleRate, UInt32 numChannels)
{
...
, m_outputBuffers(nullptr)
{
    ...
    if (initializeAudioSession)
    {
        ...
        // METHCLA_THROW_IF_ERROR(AudioSessionAddPropertyListener(kAudioSessionProperty_AudioRouteChange,
        // propListener, self), "couldn't set property listener");

        // NOTE: This needs to be called *before* trying to determine the
        //       number of input/output channels.
        METHCLA_THROW_IF_ERROR(AudioSessionSetActive(true),
    ...
    if (options.bufferSize >= 0)
    {
        ...
        METHCLA_THROW_IF_ERROR(
            AudioSessionSetProperty(
                ...
                sizeof(hwBufferDuration), &hwBufferDuration),
    ...
    METHCLA_THROW_IF_ERROR(
        AudioSessionGetProperty(
            kAudioSessionProperty_CurrentHardwareIOBufferDuration, &outSize,
            &hwBufferDuration),
    ...
    METHCLA_THROW_IF_ERROR(AudioComponentInstanceNew(comp, &m_rioUnit),
                           "couldn't open the remote I/O unit");

    // Enable output
    UInt32 enableOutput = 1;
    METHCLA_THROW_IF_ERROR(
        AudioUnitSetProperty(m_rioUnit, kAudioOutputUnitProperty_EnableIO,
                             kAudioUnitScope_Output, kOutputBus, &enableOutput,
                             sizeof(enableOutput)),
...
#if !TARGET_IPHONE_SIMULATOR // Doesn't work on the simulator
    {
        ...
        METHCLA_THROW_IF_ERROR(
            AudioUnitSetProperty(
                m_rioUnit, kAudioUnitProperty_MaximumFramesPerSlice,
                kAudioUnitScope_Global, kInputBus, &maxFPS, sizeof(maxFPS)),
            "couldn't set AudioUnit buffer size");

        METHCLA_THROW_IF_ERROR(
            AudioUnitSetProperty(
                m_rioUnit, kAudioUnitProperty_MaximumFramesPerSlice,
                kAudioUnitScope_Global, kOutputBus, &maxFPS, sizeof(maxFPS)),
            "couldn't set AudioUnit buffer size");
    }
    ...
    //
    // [1]
    // https://developer.apple.com/library/ios/documentation/AudioUnit/Reference/AudioUnitPropertiesReference/Reference/reference.html#//apple_ref/c/econst/kAudioUnitProperty_MaximumFramesPerSlice
    ...
    if (m_numInputs > 0)
    {
        ...
        METHCLA_THROW_IF_ERROR(
            AudioUnitGetProperty(
                m_rioUnit, kAudioUnitProperty_MaximumFramesPerSlice,
                kAudioUnitScope_Global, kOutputBus, &maxFPS, &outSize),
    ...
    if (m_numInputs > 0)
    {
        ...
        METHCLA_THROW_IF_ERROR(
            AudioUnitSetProperty(m_rioUnit, kAudioUnitProperty_StreamFormat,
    ...
    METHCLA_THROW_IF_ERROR(
        AudioUnitSetProperty(m_rioUnit, kAudioUnitProperty_StreamFormat,
                             kAudioUnitScope_Input, kOutputBus, &outputFormat,
                             sizeof(outputFormat)),
        "couldn't set the remote I/O unit's output client format");

    // outSize = sizeof(outputFormat);
    // METHCLA_THROW_IF_ERROR(
    //     AudioUnitGetProperty(m_rioUnit, kAudioUnitProperty_StreamFormat,
    //     kAudioUnitScope_Input, kOutputBus, &outputFormat, &outSize)
    ...
    if (m_numInputs > 0)
    {
        ...
        METHCLA_THROW_IF_ERROR(
            AudioUnitSetProperty(m_rioUnit,
    ...
    METHCLA_THROW_IF_ERROR(
        AudioUnitSetProperty(m_rioUnit, kAudioUnitProperty_SetRenderCallback,
                             kAudioUnitScope_Global, kOutputBus,
                             &outputCallback, sizeof(outputCallback)),
        "couldn't set remote i/o output callback");

    // Initialize audio unit
    METHCLA_THROW_IF_ERROR(AudioUnitInitialize(m_rioUnit),
...
{
    METHCLA_THROW_IF_ERROR(AudioOutputUnitStart(m_rioUnit),
                           "couldn't start remote i/o unit");
}

void RemoteIODriver::stop()
{
    METHCLA_THROW_IF_ERROR(AudioOutputUnitStop(m_rioUnit),
                           "couldn't start remote i/o unit");
}

void RemoteIODriver::InterruptionCallback(void*  inClientData,
    ...
    UInt32 inNumberFrames, AudioBufferList* /* ioData */)
{
    ...
    OSStatus err = AudioUnitRender(self->m_rioUnit, ioActionFlags, inTimeStamp,
    ...
    for (UInt32 bufCount = 0; bufCount < bufferList.mNumberBuffers; bufCount++)
    {
        ...
        const AudioSampleType* pcm =
            static_cast<AudioSampleType*>(bufferList.mBuffers[bufCount].mData);
        ...
        for (UInt32 curChan = 0; curChan < numInputs; curChan++)
        {
            ...
            {
                inputBuffers[curChan][curFrame] =
                    pcm[curFrame * numInputs + curChan] / 32768.f;
            }
        }
    }
    ...
    UInt32 inNumberFrames, AudioBufferList* ioData)
{
    ...
    for (UInt32 bufCount = 0; bufCount < ioData->mNumberBuffers; bufCount++)
    {
        ...
        AudioSampleType* pcm =
            static_cast<AudioSampleType*>(ioData->mBuffers[bufCount].mData);

        // Run DSP graph
        self->process(inTimeStamp->mHostTime *
                          CAHostTimeBase::GetInverseFrequency(),
                      inNumberFrames, inputBuffers, outputBuffers);
        ...
        for (UInt32 curChan = 0; curChan < numOutputs; curChan++)
        {
            for (UInt32 curFrame = 0; curFrame < inNumberFrames; curFrame++)
            {
                ...
                {
                    pcm[curFrame * numOutputs + curChan] =
                        inputBuffers[curChan][curFrame] * 32767.f;
                }
                else
                {
...
AudioUnit RemoteIODriver::audioUnit() { return m_rioUnit; }
    ...
    const Methcla_AudioDriverOptions* options)
{
    return Methcla::API::wrapAudioDriver(
        new RemoteIODriver(Methcla::API::convertOptions(options), false));
}
