...
static void recorderCallback(SLAndroidSimpleBufferQueueItf bq, void* context)
{
    ...
    if (p->intervals < INITIAL_DELAY)
    {
        ...
        if (p->inputTime.tv_sec + p->inputTime.tv_nsec > 0)
        {
            ...
            {
                __sync_bool_compare_and_swap(&p->intervals, p->intervals,
                                             p->intervals + 1);
                if (p->intervals < INITIAL_DELAY)
                {
...
static void playerCallback(SLAndroidSimpleBufferQueueItf bq, void* context)
{
    ...
    if (p->callbacks < 2)
    {
        ...
        {
            sem_post(
                &p->semReady); // Start reading input on the second invocation.
        }
    }
    if (p->readIndex < 0 &&
    ...
    if (p->readIndex >= 0)
    { // Render with input if available.
        ...
        p->readIndex =
            (p->readIndex + p->callbackBufferFrames) % p->totalBufferFrames;
    }
    else
    { // Render with empty input when input is not yet availabe.
        p->callback(p->context, p->sampleRate, p->callbackBufferFrames,
                    p->inputChannels, p->dummyBuffer, p->outputChannels,
                    p->outputBuffer + p->outputIndex * p->outputChannels);
    }
    (*bq)->Enqueue(bq, p->outputBuffer + p->outputIndex * p->outputChannels,
                   p->callbackBufferFrames * p->outputChannels * sizeof(short));
    p->outputIndex =
        (p->outputIndex + p->callbackBufferFrames) % p->totalBufferFrames;
}
...
static SLresult openSLCreateEngine(OPENSL_STREAM* p)
{
    ...
    SLresult result = slCreateEngine(&p->engineObject, 0, NULL, engineIIDCount,
                                     engineIIDs, engineReqs);
    ...
    return (*p->engineObject)
        ->GetInterface(p->engineObject, SL_IID_ENGINE, &p->engineEngine);
}
...
static SLresult openSLRecOpen(OPENSL_STREAM* p, SLuint32 sr)
{
    ...
    SLresult            result =
        (*p->engineEngine)
            ->CreateAudioRecorder(p->engineEngine, &p->recorderObject,
    ...
    result = (*p->recorderObject)
                 ->GetInterface(p->recorderObject, SL_IID_RECORD,
                                &p->recorderRecord);
    if (SL_RESULT_SUCCESS != result)
        return result;
    ...
    result =
        (*p->recorderBufferQueue)
            ->RegisterCallback(p->recorderBufferQueue, recorderCallback, p);
    return result;
}
...
static SLresult openSLPlayOpen(OPENSL_STREAM* p, SLuint32 sr)
{
    ...
    SLresult            result =
        (*p->engineEngine)
            ->CreateOutputMix(p->engineEngine, &p->outputMixObject, mixIIDCount,
                              mixIIDs, mixReqs);
    if (result != SL_RESULT_SUCCESS)
        return result;

    result =
        (*p->outputMixObject)->Realize(p->outputMixObject, SL_BOOLEAN_FALSE);
    if (result != SL_RESULT_SUCCESS)
        return result;
    ...
    format_pcm.channelMask =
        p->outputChannels > 1 ? SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT
    ...
    const SLInterfaceID playIIDs[] = {SL_IID_PLAY,
                                      SL_IID_ANDROIDSIMPLEBUFFERQUEUE};
    const SLboolean     playReqs[] = {SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE};
    result =
        (*p->engineEngine)
            ->CreateAudioPlayer(p->engineEngine, &p->playerObject, &audioSrc,
                                &audioSnk, playIIDCount, playIIDs, playReqs);
    if (result != SL_RESULT_SUCCESS)
        return result;
    ...
    result = (*p->playerObject)
                 ->GetInterface(p->playerObject, SL_IID_PLAY, &p->playerPlay);
    if (result != SL_RESULT_SUCCESS)
        return result;

    result =
        (*p->playerObject)
            ->GetInterface(p->playerObject, SL_IID_ANDROIDSIMPLEBUFFERQUEUE,
                           &p->playerBufferQueue);
    if (result != SL_RESULT_SUCCESS)
        return result;

    result = (*p->playerBufferQueue)
                 ->RegisterCallback(p->playerBufferQueue, playerCallback, p);
    return result;
}
...
OPENSL_STREAM* opensl_open(int sampleRate, int inChans, int outChans,
                           int callbackBufferFrames, opensl_process_t proc,
                           void* context)
{
    if (sampleRate <= 0)
        return NULL;
    ...
    if (p->inputChannels > 0)
    {
        if (!(openSLRecOpen(p, srmillihz) == SL_RESULT_SUCCESS &&
              (p->inputBuffer =
                   (short*)calloc(inputBufferSize(p), sizeof(short))) &&
              (p->dummyBuffer = (short*)calloc(
                   callbackBufferFrames * p->inputChannels, sizeof(short)))))
        {
            opensl_close(p);
            return NULL;
        }
    ...
    if (p->outputChannels > 0)
    {
        if (!(openSLPlayOpen(p, srmillihz) == SL_RESULT_SUCCESS &&
              (p->outputBuffer =
                   (short*)calloc(outputBufferSize(p), sizeof(short)))))
        {
            opensl_close(p);
            return NULL;
        }
...
int opensl_start(OPENSL_STREAM* p)
{
    ...
    while (!sem_trywait(&p->semReady))
        ; // Clear semaphore, just in case.
    ...
    if ((*p->playerPlay)->SetPlayState(p->playerPlay, SL_PLAYSTATE_PLAYING) !=
        SL_RESULT_SUCCESS)
    {
        opensl_pause(p);
        return -1;
    ...
    if (p->recorderRecord)
    {
        ...
        sem_wait(&p->semReady);
        if ((*p->recorderRecord)
                ->SetRecordState(p->recorderRecord, SL_RECORDSTATE_RECORDING) !=
            SL_RESULT_SUCCESS)
        {
            opensl_pause(p);
            return -1;
...
void opensl_pause(OPENSL_STREAM* p)
{
    if (p->isRunning)
    {
        if (p->recorderRecord)
        {
            ...
            (*p->recorderRecord)
                ->SetRecordState(p->recorderRecord, SL_RECORDSTATE_STOPPED);
        }
