...
using methcla_boost::timer::nanosecond_type;
using methcla_boost::timer::cpu_times;
...
namespace
{
  ...
  //  be as low as 10, although will be 15 for many common platforms.
  {
    if (places > 9)
      ...
      places = methcla_boost::timer::default_places;
 
    methcla_boost::io::ios_flags_saver ifs(os);
    methcla_boost::io::ios_precision_saver ips(os);
...
methcla_boost::long_long_type query_performance_frequency()
{
    LARGE_INTEGER li;
...
void get_cpu_times( methcla_boost::timer::cpu_times& current )
{
    static const methcla_boost::long_long_type freq = query_performance_frequency();

    LARGE_INTEGER li;
    ::QueryPerformanceCounter( &li ); // never fails

    methcla_boost::long_long_type ctr = li.QuadPart;

    methcla_boost::long_long_type const nano = INT64_C( 1000000000 ); // ns

    // ctr * nano / freq, but with less overflow

    methcla_boost::long_long_type whole = (ctr / freq) * nano;
    methcla_boost::long_long_type part  = (ctr % freq) * nano / freq;

    current.wall = whole + part;
    current.user = methcla_boost::timer::nanosecond_type( -1 );
    current.system = methcla_boost::timer::nanosecond_type( -1 );
    ...
    current.system = ( ( methcla_boost::timer::nanosecond_type( kernel.dwHighDateTime ) << 32 ) + kernel.dwLowDateTime ) * 100;
    current.user = ( ( methcla_boost::timer::nanosecond_type( user.dwHighDateTime ) << 32 ) + user.dwLowDateTime ) * 100;
...
methcla_boost::int_least64_t tick_factor()
{
    methcla_boost::int_least64_t tf = ::sysconf( _SC_CLK_TCK );
    if( tf <= 0 ) return -1;
...
void get_cpu_times( methcla_boost::timer::cpu_times& current )
{
    current.wall = methcla_boost::timer::nanosecond_type( -1 );
    current.user = methcla_boost::timer::nanosecond_type( -1 );
    current.system = methcla_boost::timer::nanosecond_type( -1 );

    static methcla_boost::int_least64_t tf = tick_factor();
    ...
    current.wall = methcla_boost::timer::nanosecond_type( c ) * tf;
    current.system = methcla_boost::timer::nanosecond_type( tm.tms_stime + tm.tms_cstime ) * tf;
    current.user = methcla_boost::timer::nanosecond_type( tm.tms_utime + tm.tms_cutime ) * tf;
}
