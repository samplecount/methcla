...
void system_crypt_random(void* buf, std::size_t len, methcla_boost::system::error_code* ec)
{
#if defined(BOOST_FILESYSTEM_POSIX_API)
    ...
    if (BOOST_UNLIKELY(err != 0))
        emit_error(err, ec, "methcla_boost::filesystem::unique_path");
    ...
    methcla_boost::winapi::NTSTATUS_ status = methcla_boost::winapi::BCryptOpenAlgorithmProvider(&handle, methcla_boost::winapi::BCRYPT_RNG_ALGORITHM_, nullptr, 0);
    if (BOOST_UNLIKELY(status != 0))
    {
    fail:
        emit_error(translate_ntstatus(status), ec, "methcla_boost::filesystem::unique_path");
        return;
    }

    status = methcla_boost::winapi::BCryptGenRandom(handle, static_cast< methcla_boost::winapi::PUCHAR_ >(buf), static_cast< methcla_boost::winapi::ULONG_ >(len), 0);
    ...
    methcla_boost::winapi::HCRYPTPROV_ handle;
    methcla_boost::winapi::DWORD_ err = 0u;
    if (BOOST_UNLIKELY(!methcla_boost::winapi::CryptAcquireContextW(&handle, nullptr, nullptr, methcla_boost::winapi::PROV_RSA_FULL_, methcla_boost::winapi::CRYPT_VERIFYCONTEXT_ | methcla_boost::winapi::CRYPT_SILENT_)))
    {
        err = methcla_boost::winapi::GetLastError();
    ...
    methcla_boost::winapi::BOOL_ gen_ok = methcla_boost::winapi::CryptGenRandom(handle, static_cast< methcla_boost::winapi::DWORD_ >(len), static_cast< methcla_boost::winapi::BYTE_* >(buf));

    if (BOOST_UNLIKELY(!gen_ok))
        err = methcla_boost::winapi::GetLastError();

    methcla_boost::winapi::CryptReleaseContext(handle, 0);
