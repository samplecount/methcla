...
using methcla_boost::filesystem::detail::colon;
using methcla_boost::filesystem::detail::questionmark;

inline bool is_alnum(wchar_t c)
{
    return methcla_boost::filesystem::detail::is_letter(c) || (c >= L'0' && c <= L'9');
}
...
inline size_type find_separator(const wchar_t* p, size_type size) noexcept
{
    ...
    for (; pos < size; ++pos)
    {
        ...
        if (methcla_boost::filesystem::detail::is_directory_separator(c))
            break;
    }
    return pos;
}
...
std::locale default_locale()
{
    ...
    return std::locale(global_loc, new methcla_boost::filesystem::detail::windows_file_codecvt());
#elif defined(BOOST_FILESYSTEM_DETAIL_USE_UTF8_CODECVT_FACET)
    std::locale global_loc = std::locale();
    return std::locale(global_loc, new methcla_boost::filesystem::detail::utf8_codecvt_facet());
#else // Other POSIX
...
const methcla_boost::filesystem::path* g_dot_path = nullptr;
const methcla_boost::filesystem::path* g_dot_dot_path = nullptr;

inline void schedule_path_locale_cleanup() noexcept
{
}

inline methcla_boost::filesystem::path const& get_dot_path()
{
#if !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    atomic_ns::atomic_ref< const methcla_boost::filesystem::path* > a(g_dot_path);
    const methcla_boost::filesystem::path* p = a.load(atomic_ns::memory_order_acquire);
    if (BOOST_UNLIKELY(!p))
    {
        const methcla_boost::filesystem::path* new_p = new methcla_boost::filesystem::path(dot_path_literal);
        if (a.compare_exchange_strong(p, new_p, atomic_ns::memory_order_acq_rel, atomic_ns::memory_order_acquire))
            p = new_p;
        else
            delete new_p;
    ...
    return *p;
#else // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    const methcla_boost::filesystem::path* p = g_dot_path;
    if (BOOST_UNLIKELY(!p))
        g_dot_path = p = new methcla_boost::filesystem::path(dot_path_literal);
    return *p;
#endif // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
}

inline methcla_boost::filesystem::path const& get_dot_dot_path()
{
#if !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    atomic_ns::atomic_ref< const methcla_boost::filesystem::path* > a(g_dot_dot_path);
    const methcla_boost::filesystem::path* p = a.load(atomic_ns::memory_order_acquire);
    if (BOOST_UNLIKELY(!p))
    {
        const methcla_boost::filesystem::path* new_p = new methcla_boost::filesystem::path(dot_dot_path_literal);
        if (a.compare_exchange_strong(p, new_p, atomic_ns::memory_order_acq_rel, atomic_ns::memory_order_acquire))
            p = new_p;
        else
            delete new_p;
    ...
    return *p;
#else // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
    const methcla_boost::filesystem::path* p = g_dot_dot_path;
    if (BOOST_UNLIKELY(!p))
        g_dot_dot_path = p = new methcla_boost::filesystem::path(dot_dot_path_literal);
    return *p;
#endif // !defined(BOOST_FILESYSTEM_SINGLE_THREADED)
}
...
BOOST_FILESYSTEM_INIT_FUNC init_path_globals()
{
    ...
    methcla_boost::filesystem::codecvt_error_category();
#endif
...
const methcla_boost::filesystem::path g_dot_path(dot_path_literal);
BOOST_FILESYSTEM_INIT_PRIORITY(BOOST_FILESYSTEM_PATH_GLOBALS_INIT_PRIORITY)
const methcla_boost::filesystem::path g_dot_dot_path(dot_dot_path_literal);
...
inline methcla_boost::filesystem::path const& get_dot_path()
{
    static const methcla_boost::filesystem::path g_dot_path(dot_path_literal);
    return g_dot_path;
}

inline methcla_boost::filesystem::path const& get_dot_dot_path()
{
    static const methcla_boost::filesystem::path g_dot_dot_path(dot_dot_path_literal);
    return g_dot_dot_path;
}
