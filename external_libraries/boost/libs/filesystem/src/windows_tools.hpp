...
using unique_handle = methcla_boost::scope::unique_resource< HANDLE, handle_deleter, handle_resource_traits >;
...
inline methcla_boost::filesystem::perms make_permissions(methcla_boost::filesystem::path const& p, DWORD attr)
{
    methcla_boost::filesystem::perms prms = methcla_boost::filesystem::owner_read | methcla_boost::filesystem::group_read | methcla_boost::filesystem::others_read;
    if ((attr & FILE_ATTRIBUTE_READONLY) == 0u)
        prms |= methcla_boost::filesystem::owner_write | methcla_boost::filesystem::group_write | methcla_boost::filesystem::others_write;
    methcla_boost::filesystem::path ext = detail::path_algorithms::extension_v4(p);
    ...
    if (equal_extension(q, L".exe", L".EXE") || equal_extension(q, L".com", L".COM") || equal_extension(q, L".bat", L".BAT") || equal_extension(q, L".cmd", L".CMD"))
        prms |= methcla_boost::filesystem::owner_exe | methcla_boost::filesystem::group_exe | methcla_boost::filesystem::others_exe;
    return prms;
}

ULONG get_reparse_point_tag_ioctl(HANDLE h, methcla_boost::filesystem::path const& p, methcla_boost::system::error_code* ec);
...
struct io_status_block
{
    union
    {
        methcla_boost::winapi::NTSTATUS_ Status;
        PVOID Pointer;
    };
...
typedef methcla_boost::winapi::NTSTATUS_ (NTAPI NtCreateFile_t)(
...
typedef methcla_boost::winapi::NTSTATUS_ (NTAPI NtQueryDirectoryFile_t)(
