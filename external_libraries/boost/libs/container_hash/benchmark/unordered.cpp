...
#if SIZE_MAX > UINT32_MAX
        methcla_boost::uint64_t h = 0xCBF29CE484222325ull;
#else
        methcla_boost::uint64_t h = 0x811C9DC5u;
#endif
...
template<class H, class V> void test_hash_speed( int N, V const& v )
{
    ...
    std::string hash = methcla_boost::core::type_name<H>();
...
{
    methcla_boost::unordered_set<std::size_t> s;
    H const h;
...
{
    methcla_boost::unordered_set<K, H> s( 0 );
    test4( N, v, methcla_boost::core::type_name<H>().c_str(), s );
}
...
int main()
{
...

    {
        ...
        methcla_boost::detail::splitmix64 rnd;
    ...
    test_hash_speed<methcla_boost::hash<std::string> >( N * 16, v );
...

    {
        ...
        {
            methcla_boost::unordered_set<std::string> s;
        ...
        test_hash_collision<methcla_boost::hash<std::string> >( N * 16, v, n );
    ...
    test_container_speed<K, methcla_boost::hash<std::string> >( N, v );
