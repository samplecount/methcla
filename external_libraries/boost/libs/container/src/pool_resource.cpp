   ...
   : public block_slist_base<>
{
   ...
   {
      //Limit max value
      std::size_t blocks_per_chunk = methcla_boost::container::dtl::min_value(max_blocks_per_chunk, next_blocks_per_chunk);
      //Avoid overflow
      blocks_per_chunk = methcla_boost::container::dtl::min_value(blocks_per_chunk, std::size_t(-1)/pool_block);
...
void pool_resource::priv_limit_option(std::size_t &val, std::size_t min, std::size_t max) //static
{
   if(!val){
      ...
      val = val < min ? min : methcla_boost::container::dtl::min_value(val, max);
   }
}
...
std::size_t pool_resource::priv_pool_index(std::size_t block_size) //static
{
   ...
   block_size = methcla_boost::container::dtl::max_value(block_size, pool_options_minimum_largest_required_pool_block);
