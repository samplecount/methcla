   ...
   : public memory_resource
{
   ...
   {  return methcla_boost::container::dtl::operator_new_raw_allocate(bytes, alignment);  }

   void do_deallocate(void* p, std::size_t bytes, std::size_t alignment) BOOST_OVERRIDE
   {  return methcla_boost::container::dtl::operator_delete_raw_deallocate(p, bytes, alignment);  }
...
{
   return &methcla_boost::container::dtl::singleton_default<new_delete_resource_imp>::instance();
}

BOOST_CONTAINER_DECL memory_resource* null_memory_resource() BOOST_NOEXCEPT
{
   return &methcla_boost::container::dtl::singleton_default<null_memory_resource_imp>::instance();
}

#if defined(BOOST_NO_CXX11_HDR_ATOMIC)

static memory_resource *default_memory_resource =
   &methcla_boost::container::dtl::singleton_default<new_delete_resource_imp>::instance();
