...
public:
    ...
    template<class U>
    BOOST_SP_CXX20_CONSTEXPR intrusive_ptr( intrusive_ptr<U> const & rhs, typename methcla_boost::detail::sp_enable_if_convertible<U,T>::type = methcla_boost::detail::sp_empty() )
    : px( rhs.get() )
    {
...
// Move support
    ...
    template<class U>
    BOOST_SP_CXX20_CONSTEXPR intrusive_ptr(intrusive_ptr<U> && rhs, typename methcla_boost::detail::sp_enable_if_convertible<U,T>::type = methcla_boost::detail::sp_empty())
    : px( rhs.px )
    {
        rhs.px = 0;
    }
...
template< class T > std::size_t hash_value( methcla_boost::intrusive_ptr<T> const & p ) noexcept
{
    return methcla_boost::hash< T* >()( p.get() );
}
...
template<class T> struct hash< ::methcla_boost::intrusive_ptr<T> >
{
    std::size_t operator()( ::methcla_boost::intrusive_ptr<T> const & p ) const noexcept
    {
