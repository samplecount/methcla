...
public:
    ...
    T* operator->() const noexcept {
        return methcla_boost::to_address(p_);
    }
...
template<class A>
inline void
sp_alloc_clear(A& a, typename methcla_boost::allocator_pointer<A>::type p, std::size_t,
    std::false_type)
{
    methcla_boost::alloc_destroy(a, methcla_boost::to_address(p));
}

template<class A>
inline void
sp_alloc_clear(A& a, typename methcla_boost::allocator_pointer<A>::type p,
...
#endif
    methcla_boost::alloc_destroy_n(a, methcla_boost::first_scalar(methcla_boost::to_address(p)),
...
class sp_alloc_make {
public:
    typedef typename methcla_boost::allocator_rebind<A,
        typename sp_alloc_value<T>::type>::type allocator;

private:
    typedef methcla_boost::alloc_deleter<T, A> deleter;
...
private:
    typedef typename methcla_boost::allocator_pointer<allocator>::type pointer;
...
allocate_unique(const A& alloc)
{
    ...
    methcla_boost::alloc_construct(c.state(), c.get());
    return c.release();
}
...
{
    detail::sp_alloc_make<T, A> c(alloc, 1);
    methcla_boost::alloc_construct(c.state(), c.get(), std::forward<Args>(args)...);
    return c.release();
}
...
{
    detail::sp_alloc_make<T, A> c(alloc, 1);
    methcla_boost::alloc_construct(c.state(), c.get(), std::move(value));
    return c.release();
}
...
{
    return methcla_boost::allocate_unique<T, noinit_adaptor<A> >(alloc);
}
...
allocate_unique(const A& alloc, std::size_t size)
{
    ...
    methcla_boost::alloc_construct_n(c.state(), methcla_boost::first_scalar(c.get()),
...
{
    return methcla_boost::allocate_unique<T, noinit_adaptor<A> >(alloc, size);
}
    ...
    const typename std::remove_extent<T>::type& value)
{
    detail::sp_alloc_make<T, A> c(alloc, size);
    methcla_boost::alloc_construct_n(c.state(), methcla_boost::first_scalar(c.get()),
        size * detail::sp_alloc_size<T>::value, methcla_boost::first_scalar(&value),
    ...
    const typename std::remove_extent<T>::type& value)
{
    detail::sp_alloc_make<T, A> c(alloc, std::extent<T>::value);
    methcla_boost::alloc_construct_n(c.state(), methcla_boost::first_scalar(c.get()),
        detail::sp_alloc_size<T>::value, methcla_boost::first_scalar(&value),
        detail::sp_alloc_size<typename std::remove_extent<T>::type>::value);
    return c.release();
}
