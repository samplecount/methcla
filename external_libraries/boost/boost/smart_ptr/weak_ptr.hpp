...
//
    ...
    template<class Y>
    weak_ptr( weak_ptr<Y> const & r, typename methcla_boost::detail::sp_enable_if_convertible<Y,T>::type = methcla_boost::detail::sp_empty() )
    ...
    template<class Y>
    weak_ptr( weak_ptr<Y> && r, typename methcla_boost::detail::sp_enable_if_convertible<Y,T>::type = methcla_boost::detail::sp_empty() )
    noexcept : px( r.lock().get() ), pn( static_cast< methcla_boost::detail::weak_count && >( r.pn ) )
    {
        methcla_boost::detail::sp_assert_convertible< Y, T >();
        r.px = 0;
    }
    ...
    weak_ptr( weak_ptr && r )
    noexcept : px( r.px ), pn( static_cast< methcla_boost::detail::weak_count && >( r.pn ) )
    {
        r.px = 0;
    }
    ...
    template<class Y>
    weak_ptr( shared_ptr<Y> const & r, typename methcla_boost::detail::sp_enable_if_convertible<Y,T>::type = methcla_boost::detail::sp_empty() )
    ...
    {
        return shared_ptr<T>( *this, methcla_boost::detail::sp_nothrow_tag() );
    }
...
private:
    ...
    methcla_boost::detail::weak_count pn; // reference counter
...
template< class T > std::size_t hash_value( methcla_boost::weak_ptr<T> const & p ) noexcept
{
...
template<class T> struct hash< ::methcla_boost::weak_ptr<T> >
{
    std::size_t operator()( ::methcla_boost::weak_ptr<T> const & p ) const noexcept
    {
        return p.owner_hash_value();
    }
};

template<class T> struct equal_to< ::methcla_boost::weak_ptr<T> >
{
    bool operator()( ::methcla_boost::weak_ptr<T> const & a, ::methcla_boost::weak_ptr<T> const & b ) const noexcept
    {
