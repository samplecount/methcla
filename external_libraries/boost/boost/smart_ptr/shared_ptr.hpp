...
template< class X, class Y, class T > inline void sp_enable_shared_from_this( methcla_boost::shared_ptr<X> const * ppx, Y const * py, methcla_boost::enable_shared_from_this< T > const * pe )
{
    if( pe != 0 )
    {
...
template< class X, class Y > inline void sp_enable_shared_from_this( methcla_boost::shared_ptr<X> * ppx, Y const * py, methcla_boost::enable_shared_from_raw const * pe );
...
template< class T, class Y > inline void sp_pointer_construct( methcla_boost::shared_ptr< T > * ppx, Y * p, methcla_boost::detail::shared_count & pn )
{
    methcla_boost::detail::shared_count( p ).swap( pn );
    methcla_boost::detail::sp_enable_shared_from_this( ppx, p, p );
}

template< class T, class Y > inline void sp_pointer_construct( methcla_boost::shared_ptr< T[] > * /*ppx*/, Y * p, methcla_boost::detail::shared_count & pn )
{
    sp_assert_convertible< Y[], T[] >();
    methcla_boost::detail::shared_count( p, methcla_boost::checked_array_deleter< T >() ).swap( pn );
}

template< class T, std::size_t N, class Y > inline void sp_pointer_construct( methcla_boost::shared_ptr< T[N] > * /*ppx*/, Y * p, methcla_boost::detail::shared_count & pn )
{
...
template< class T, class Y > inline void sp_deleter_construct( methcla_boost::shared_ptr< T > * ppx, Y * p )
{
    methcla_boost::detail::sp_enable_shared_from_this( ppx, p, p );
}
...
template< class T, std::size_t N, class Y > inline void sp_deleter_construct( methcla_boost::shared_ptr< T[N] > * /*ppx*/, Y * /*p*/ )
{
    sp_assert_convertible< Y[N], T[N] >();
}
...
public:
    ...
    typedef typename methcla_boost::detail::sp_element< T >::type element_type;
    ...
    constexpr shared_ptr( methcla_boost::detail::sp_internal_constructor_tag, element_type * px_, methcla_boost::detail::shared_count const & pn_ ) noexcept : px( px_ ), pn( pn_ )
    {
    }

    constexpr shared_ptr( methcla_boost::detail::sp_internal_constructor_tag, element_type * px_, methcla_boost::detail::shared_count && pn_ ) noexcept : px( px_ ), pn( std::move( pn_ ) )
    {
    }
    ...
    {
        methcla_boost::detail::sp_pointer_construct( this, p, pn );
    }
    ...
    {
        methcla_boost::detail::sp_deleter_construct( this, p );
    }
...
// ... except in C++0x, move disables the implicit copy
    ...
    {
        methcla_boost::detail::sp_assert_convertible< Y, T >();
    ...
    template<class Y>
    shared_ptr( weak_ptr<Y> const & r, methcla_boost::detail::sp_nothrow_tag )
    noexcept : px( 0 ), pn( r.pn, methcla_boost::detail::sp_nothrow_tag() )
    {
        if( !pn.empty() )
        {
            px = r.px;
    ...
    template<class Y>
    shared_ptr( shared_ptr<Y> const & r, typename methcla_boost::detail::sp_enable_if_convertible<Y,T>::type = methcla_boost::detail::sp_empty() )
    ...
    explicit shared_ptr( std::auto_ptr<Y> & r ): px(r.get()), pn()
    {
        ...
        Y * tmp = r.get();
        pn = methcla_boost::detail::shared_count( r );

        methcla_boost::detail::sp_deleter_construct( this, tmp );
    }
    ...
    template< class Y, class D >
    shared_ptr( methcla_boost::movelib::unique_ptr< Y, D > r ): px( r.get() ), pn()
    {
        methcla_boost::detail::sp_assert_convertible< Y, T >();

        typename methcla_boost::movelib::unique_ptr< Y, D >::pointer tmp = r.get();
    ...
    template<class Y, class D>
    shared_ptr & operator=( methcla_boost::movelib::unique_ptr<Y, D> r )
    {
        // this_type( static_cast< unique_ptr<Y, D> && >( r ) ).swap( *this );

        methcla_boost::detail::sp_assert_convertible< Y, T >();

        typename methcla_boost::movelib::unique_ptr< Y, D >::pointer p = r.get();
        ...
        {
            tmp.px = p;
            tmp.pn = methcla_boost::detail::shared_count( r );

            methcla_boost::detail::sp_deleter_construct( &tmp, p );
        }
...
// Move support
    ...
    shared_ptr( shared_ptr && r ) noexcept : px( r.px ), pn( static_cast< methcla_boost::detail::shared_count && >( r.pn ) )
    {
        r.px = 0;
    }

    template<class Y>
    shared_ptr( shared_ptr<Y> && r, typename methcla_boost::detail::sp_enable_if_convertible<Y,T>::type = methcla_boost::detail::sp_empty() )
    noexcept : px( r.px ), pn( static_cast< methcla_boost::detail::shared_count && >( r.pn ) )
    {
        methcla_boost::detail::sp_assert_convertible< Y, T >();
        r.px = 0;
    }
    ...
    typename methcla_boost::detail::sp_dereference< T >::type operator* () const BOOST_SP_NOEXCEPT_WITH_ASSERT
    {
        BOOST_ASSERT( px != 0 );
        return *px;
    }
    
    typename methcla_boost::detail::sp_member_access< T >::type operator-> () const BOOST_SP_NOEXCEPT_WITH_ASSERT
    {
        BOOST_ASSERT( px != 0 );
        return px;
    }
    
    typename methcla_boost::detail::sp_array_access< T >::type operator[] ( std::ptrdiff_t i ) const BOOST_SP_NOEXCEPT_WITH_ASSERT
    {
        BOOST_ASSERT( px != 0 );
        BOOST_ASSERT( i >= 0 && ( i < methcla_boost::detail::sp_extent< T >::value || methcla_boost::detail::sp_extent< T >::value == 0 ) );

        return static_cast< typename methcla_boost::detail::sp_array_access< T >::type >( px[ i ] );
    }
    ...
    void * _internal_get_deleter( methcla_boost::detail::sp_typeinfo_ const & ti ) const noexcept
    {
        return pn.get_deleter( ti );
    }

    void * _internal_get_local_deleter( methcla_boost::detail::sp_typeinfo_ const & ti ) const noexcept
    {
    ...
    methcla_boost::detail::shared_count _internal_count() const noexcept
    {
        return pn;
    }
...
private:
    ...
    methcla_boost::detail::shared_count pn;    // reference counter
...
public:
    ...
    {
        return methcla_boost::detail::basic_get_deleter<D>( deleter_ );
    }
...
{
    D * d = methcla_boost::detail::basic_get_deleter<D>( p );

    if( d == 0 )
    {
        d = methcla_boost::detail::basic_get_local_deleter( d, p );
    }

    if( d == 0 )
    {
        methcla_boost::detail::esft2_deleter_wrapper *del_wrapper = methcla_boost::detail::basic_get_deleter<methcla_boost::detail::esft2_deleter_wrapper>(p);
// The following get_deleter method call is fully qualified because
// older versions of gcc (2.95, 3.2.3) fail to compile it when written del_wrapper->get_deleter<D>()
        if(del_wrapper) d = del_wrapper->::methcla_boost::detail::esft2_deleter_wrapper::get_deleter<D>();
    }
...
{
    methcla_boost::detail::spinlock_pool<2>::scoped_lock lock( p );
    return *p;
}
...
{
    methcla_boost::detail::spinlock & sp = methcla_boost::detail::spinlock_pool<2>::spinlock_for( p );
...
template< class T > std::size_t hash_value( methcla_boost::shared_ptr<T> const & p ) noexcept
{
    return methcla_boost::hash< typename methcla_boost::shared_ptr<T>::element_type* >()( p.get() );
}
...
template<class T> struct hash< ::methcla_boost::shared_ptr<T> >
{
    std::size_t operator()( ::methcla_boost::shared_ptr<T> const & p ) const noexcept
    {
        return std::hash< typename ::methcla_boost::shared_ptr<T>::element_type* >()( p.get() );
    }
};
