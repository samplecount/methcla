...
template< class E, class Y > inline void lsp_pointer_construct( methcla_boost::local_shared_ptr< E > * /*ppx*/, Y * p, methcla_boost::detail::local_counted_base * & pn )
{
    methcla_boost::detail::sp_assert_convertible< Y, E >();

    typedef methcla_boost::detail::local_sp_deleter< methcla_boost::checked_deleter<Y> > D;

    methcla_boost::shared_ptr<E> p2( p, D() );
...
template< class E, class Y > inline void lsp_pointer_construct( methcla_boost::local_shared_ptr< E[] > * /*ppx*/, Y * p, methcla_boost::detail::local_counted_base * & pn )
{
    ...
    typedef methcla_boost::detail::local_sp_deleter< methcla_boost::checked_array_deleter<E> > D;
...
template< class E, std::size_t N, class Y > inline void lsp_pointer_construct( methcla_boost::local_shared_ptr< E[N] > * /*ppx*/, Y * p, methcla_boost::detail::local_counted_base * & pn )
{
    methcla_boost::detail::sp_assert_convertible< Y[N], E[N] >();

    typedef methcla_boost::detail::local_sp_deleter< methcla_boost::checked_array_deleter<E> > D;

    methcla_boost::shared_ptr<E[N]> p2( p, D() );
...
template< class E, class P, class D > inline void lsp_deleter_construct( methcla_boost::local_shared_ptr< E > * /*ppx*/, P p, D const& d, methcla_boost::detail::local_counted_base * & pn )
{
    typedef methcla_boost::detail::local_sp_deleter<D> D2;

    methcla_boost::shared_ptr<E> p2( p, D2( d ) );
...
template< class E, class P, class D, class A > inline void lsp_allocator_construct( methcla_boost::local_shared_ptr< E > * /*ppx*/, P p, D const& d, A const& a, methcla_boost::detail::local_counted_base * & pn )
{
    typedef methcla_boost::detail::local_sp_deleter<D> D2;

    methcla_boost::shared_ptr<E> p2( p, D2( d ), a );
...
private:
    ...
    element_type * px;
    methcla_boost::detail::local_counted_base * pn;
...
public:
    ...
    constexpr local_shared_ptr( methcla_boost::detail::lsp_internal_constructor_tag, element_type * px_, methcla_boost::detail::local_counted_base * pn_ ) noexcept : px( px_ ), pn( pn_ )
    {
    }
    ...
    {
        methcla_boost::detail::lsp_pointer_construct( this, p, pn );
    }

    template<class Y, class D> local_shared_ptr( Y * p, D d ): px( p ), pn( 0 )
    {
        methcla_boost::detail::lsp_deleter_construct( this, p, d, pn );
    }
    ...
    {
        methcla_boost::detail::lsp_allocator_construct( this, p, d, a, pn );
    }
    ...
    template<class Y> local_shared_ptr( shared_ptr<Y> const & r,
        typename methcla_boost::detail::sp_enable_if_convertible<Y, T>::type = methcla_boost::detail::sp_empty() )
        : px( r.get() ), pn( 0 )
    {
        ...
        if( r.use_count() != 0 )
        {
            pn = new methcla_boost::detail::local_counted_impl( r._internal_count() );
        }
    }
        ...
        : px( r.get() ), pn( 0 )
    {
        ...
        if( px )
        {
            pn = new methcla_boost::detail::local_counted_impl( shared_ptr<T>( std::move(r) )._internal_count() );
        }
    }

    template< class Y, class D >
    local_shared_ptr( methcla_boost::movelib::unique_ptr< Y, D > r ); // !
    //  : px( r.get() ), pn( new methcla_boost::detail::local_counted_impl( shared_ptr<T>( std::move(r) ) ) )
    //{
    //    methcla_boost::detail::sp_assert_convertible< Y, T >();
    //}
    ...
    template<class Y> local_shared_ptr( local_shared_ptr<Y> const & r,
        typename methcla_boost::detail::sp_enable_if_convertible<Y, T>::type = methcla_boost::detail::sp_empty() ) noexcept
        : px( r.px ), pn( r.pn )
    {
        methcla_boost::detail::sp_assert_convertible< Y, T >();
    ...
    template<class Y, class D>
    local_shared_ptr & operator=( methcla_boost::movelib::unique_ptr<Y, D> r ); // !
    ...
    typename methcla_boost::detail::sp_dereference< T >::type operator* () const noexcept
    {
        return *px;
    }

    typename methcla_boost::detail::sp_member_access< T >::type operator-> () const noexcept
    {
        return px;
    }
    ...
    template<class Y, class E = typename methcla_boost::detail::sp_enable_if_convertible<T,Y>::type> operator shared_ptr<Y>() const noexcept
    {
        methcla_boost::detail::sp_assert_convertible<T, Y>();

        if( pn )
        {
            return shared_ptr<Y>( methcla_boost::detail::sp_internal_constructor_tag(), px, pn->local_cb_get_shared_count() );
        }
        else
        {
            return shared_ptr<Y>();
    ...
    template<class Y, class E = typename methcla_boost::detail::sp_enable_if_convertible<T,Y>::type> operator weak_ptr<Y>() const noexcept
    {
        methcla_boost::detail::sp_assert_convertible<T, Y>();
    ...
    {
        return std::less< methcla_boost::detail::local_counted_base* >()( pn, r.pn );
    }
...
// get_pointer() enables methcla_boost::mem_fn to recognize local_shared_ptr
...
{
    return methcla_boost::hash< typename local_shared_ptr<T>::element_type* >()( p.get() );
}
...
template<class T> struct hash< ::methcla_boost::local_shared_ptr<T> >
{
    std::size_t operator()( ::methcla_boost::local_shared_ptr<T> const & p ) const noexcept
    {
        return std::hash< typename ::methcla_boost::local_shared_ptr<T>::element_type* >()( p.get() );
    }
};
