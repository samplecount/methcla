...
private:
    typename methcla_boost::allocator_rebind<A, type>::type other_;
    std::size_t size_;
};
...
public:
    ...
    sp_array_base(const A& other, type* start, std::size_t size)
        : state_(other, size) {
        methcla_boost::alloc_construct_n(state_.allocator(),
            methcla_boost::first_scalar(start),
    ...
    sp_array_base(const A& other, type* start, std::size_t size, const U& list)
        ...
        methcla_boost::alloc_construct_n(state_.allocator(),
            methcla_boost::first_scalar(start), state_.size() * count,
            methcla_boost::first_scalar(&list), count);
    }
    ...
    void dispose() noexcept override {
        methcla_boost::alloc_destroy_n(state_.allocator(),
            methcla_boost::first_scalar(sp_array_start<type>(this)),
            state_.size() * sp_array_count<type>::value);
    }
...
{
    return methcla_boost::allocate_shared<T>(methcla_boost::noinit_adapt(allocator), count);
}
...
{
    return methcla_boost::allocate_shared<T>(methcla_boost::noinit_adapt(allocator));
}
