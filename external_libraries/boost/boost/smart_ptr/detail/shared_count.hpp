...
public:
    ...
    explicit sp_reference_wrapper( T & t): t_( methcla_boost::addressof( t ) )
    {
    }
...
public:
    ...
    template<class Y> explicit shared_count( Y * p ): pi_( 0 )
    {
        ...
        {
            methcla_boost::checked_delete( p );
            methcla_boost::throw_exception( std::bad_alloc() );
        }
    ...
    template<class Y, class D>
    explicit shared_count( methcla_boost::movelib::unique_ptr<Y, D> & r ): pi_( 0 )
    {
        typedef typename sp_convert_reference<D>::type D2;

        D2 d2( r.get_deleter() );
        pi_ = new sp_counted_impl_pd< typename methcla_boost::movelib::unique_ptr<Y, D>::pointer, D2 >( r.get(), d2 );
...
inline shared_count::shared_count( weak_count const & r ): pi_( r.pi_ )
{
    ...
    {
        methcla_boost::throw_exception( methcla_boost::bad_weak_ptr() );
    }
}
