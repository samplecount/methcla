...
private:
    ...
    typedef typename methcla_boost::remove_cv<T>::type T2;
    typedef typename methcla_boost::remove_cv<U>::type U2;

public:

    BOOST_STATIC_CONSTANT( bool, value =
        (methcla_boost::is_same<T2, U2>::value)
        || methcla_boost::is_void<T2>::value
        || methcla_boost::is_void<U2>::value
        || (methcla_boost::is_base_of<T2, U2>::value)
        || (methcla_boost::is_base_of<U2, T2>::value) );
};
...
public:
    ...
    typedef typename methcla_boost::conditional<

        methcla_boost::is_same<T2, U2>::value || methcla_boost::is_void<T2>::value || methcla_boost::is_base_of<T2, U2>::value,
        typename methcla_boost::copy_cv<T, U>::type,
        typename methcla_boost::copy_cv<U, T>::type
...
public:
    ...
    typedef typename methcla_boost::copy_cv<typename methcla_boost::copy_cv<typename composite_pointer_type<T2, U2>::type const, T>::type, U>::type type;
};

//Old compilers like MSVC-7.1 have problems using methcla_boost::conditional in
