...
//
namespace methcla_boost{ namespace type_traits_detail{
...
{
  template<typename Q> static methcla_boost::true_type  test(decltype(&Q::operator())*);
  template<typename>   static methcla_boost::false_type test(...);
...
struct is_likely_stateless_lambda<
  T,
  typename methcla_boost::enable_if_<has_one_operator_call<T>::value>::type> :
     methcla_boost::is_convertible<T, typename equivalent_function_pointer<T>::type
>{};
...
namespace methcla_boost {
   namespace type_traits_detail {
      ...
      template<typename T>
      struct is_likely_stateless_lambda : public methcla_boost::integral_constant<bool, false> {};
}}
