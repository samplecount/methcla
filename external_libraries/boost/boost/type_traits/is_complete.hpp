...
#define BOOST_TT_HAS_WORKING_IS_COMPLETE
   ...
   template <class T> struct is_complete
      : public integral_constant<bool, ::methcla_boost::is_function<typename methcla_boost::remove_reference<T>::type>::value || (sizeof(methcla_boost::detail::check_is_complete<T>(0)) != sizeof(char))> {};
   ...
   namespace detail {
      ...
      {
         template <class U, class = decltype(sizeof(methcla_boost::declval< U >())) >
   ...
   template <class T>
   struct is_complete : methcla_boost::integral_constant<bool, ::methcla_boost::is_function<typename methcla_boost::remove_reference<T>::type>::value || ::methcla_boost::detail::is_complete_imp<T>::value>
   {};
   template <class T>
   struct is_complete<T&> : methcla_boost::is_complete<T> {};
   ...
   namespace detail {
      ...
      template <class T>
      struct is_complete_impl : public methcla_boost::true_type {};

      template < >
      struct is_complete_impl<void> : public methcla_boost::false_type {};

      template <class T>
      struct is_complete_impl<T[]> : public methcla_boost::false_type {};
