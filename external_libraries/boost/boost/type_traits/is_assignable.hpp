...
namespace methcla_boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename U, typename = decltype(methcla_boost::declval<T>() = methcla_boost::declval<U>())>
   ...
   template <class T, class U> struct is_assignable : public integral_constant<bool, sizeof(detail::is_assignable_imp::test<T, U>(0)) == sizeof(methcla_boost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(methcla_boost::is_complete<T>::value, "Arguments to is_assignable must be complete types");
   };
