...
namespace methcla_boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename ...TheArgs, typename = decltype(T(methcla_boost::declval<TheArgs>()...))>
         static methcla_boost::type_traits::yes_type test(int);
         template<typename, typename...>
         static methcla_boost::type_traits::no_type test(...);

         template<typename T, typename Arg, typename = decltype(::new T(methcla_boost::declval<Arg>()))>
         static methcla_boost::type_traits::yes_type test1(int);
         template<typename, typename>
         static methcla_boost::type_traits::no_type test1(...);

         template <typename T>
         static methcla_boost::type_traits::yes_type ref_test(T);
         template <typename T>
         static methcla_boost::type_traits::no_type ref_test(...);
      };

   }

   template <class T, class ...Args> struct is_constructible : public integral_constant<bool, sizeof(detail::is_constructible_imp::test<T, Args...>(0)) == sizeof(methcla_boost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(::methcla_boost::is_complete<T>::value, "The target type must be complete in order to test for constructibility");
   };
   template <class T, class Arg> struct is_constructible<T, Arg> : public integral_constant<bool, is_destructible<T>::value && sizeof(methcla_boost::detail::is_constructible_imp::test1<T, Arg>(0)) == sizeof(methcla_boost::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(::methcla_boost::is_complete<T>::value, "The target type must be complete in order to test for constructibility");
   };
   template <class Ref, class Arg> struct is_constructible<Ref&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&>(methcla_boost::declval<Arg>())) == sizeof(methcla_boost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&&>(methcla_boost::declval<Arg>())) == sizeof(methcla_boost::type_traits::yes_type)>{};
