              ...
              , bool const xbuf_used)
{
   ...
   if(xbuf_used){
      methcla_boost::move_backward(cur_pos, cur_pos+l_data, new_pos+l_data);      
   }
   else{
      methcla_boost::adl_move_swap_ranges_backward(cur_pos, cur_pos+l_data, new_pos+l_data);      
              ...
              , bool const xbuf_used)
{
   //Move buffer to the total combination right
   if(xbuf_used){
      methcla_boost::move(cur_pos, cur_pos+l_data, new_pos);
   }
   else{
      methcla_boost::adl_move_swap_ranges(cur_pos, cur_pos+l_data, new_pos);
      //Rotate does less moves but it seems slower due to cache issues
      //rotate_gcd(first-l_block, first+len-l_block, first+len);
   }
}
      ...
      , Compare comp)
{
   ...
   if(kbuf){
      ...
      if(kbuf != l_build_buf){
         methcla_boost::move(xbuf.data()+kbuf-l_merged, xbuf.data() + kbuf, first_block-l_merged+elements_in_blocks);
      }
   }
   else{
   ...
   if(kbuf && kbuf == l_build_buf){
      ...
      methcla_boost::move(xbuf.data(), xbuf.data() + kbuf, first);
   }
   else{
   ...
   , bool merge_left)
{
   ...
   methcla_boost::movelib::ignore(l_total_combined);
   ...
   if(merge_left || !use_buf) {
      for( size_type combined_i = 0; combined_i != max_i; ) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(combined_first, combined_first + n_block_a*l_block+l_irreg1, comp));
            BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(combined_first + n_block_a*l_block+l_irreg1, combined_first + n_block_a*l_block+l_irreg1+n_block_b*l_block+l_irreg2, comp));
         if(!use_buf){
   ...
   , Compare comp)
{
   ...
   if(common_xbuf){
      if(buffer_right){
         methcla_boost::move(xbuf.data(), xbuf.data() + l_intbuf, buffer+l_data);
      }
      else{
         methcla_boost::move(xbuf.data(), xbuf.data() + l_intbuf, buffer);
      }
   }
   return buffer_right;
}
               ...
               , typename iter_size<RandIt>::type uninitialized_len)
{
   ...
   ::methcla_boost::movelib::adaptive_xbuf<value_type, RandRawIt, size_type> xbuf(uninitialized, uninitialized_len);
   ::methcla_boost::movelib::detail_adaptive::adaptive_sort_impl(first, size_type(last - first), comp, xbuf);
}
