...
BOOST_MOVE_STATIC_ASSERT((AdaptiveSortInsertionSortThreshold&(AdaptiveSortInsertionSortThreshold-1)) == 0);
   ...
   typedef ::methcla_boost::uintptr_t uintptr_t;
#else
...
{
   if(begin != with){
      ::methcla_boost::adl_move_swap_ranges(begin, end, with);
      if(key_next != key_range2)  //Avoid potential self-swapping
         ::methcla_boost::adl_move_swap(*key_next, *key_range2);
      if(key_next == key_mid){
         key_mid = key_range2;
      }
   ...
   (RandIt first1, RandIt last1, RandIt const last2, bool *const pis_range1_A, Compare comp)
{
   ...
   if(first1 != last1 && comp(*last1, last1[-1])){
      do{
         ...
         last1  = methcla_boost::movelib::lower_bound(last1, last2, *first1, comp);
   ...
   , Compare comp)
{
   ...
   ::methcla_boost::movelib::ignore(key_count);
   //assert(n_block_a || n_block_b);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted_and_unique(key_first, key_first + key_count, key_comp));
   ...
   {  //Selection sort blocks
      ...
      for ( RandIt f = first+l_irreg1; n_block_left; --n_block_left) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(f, f+l_block, comp));
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first_min, first_min + l_block, comp));
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first+l_irreg1+n_bef_irreg2*l_block, first_irr2, comp));
   ...
   for(RandItKeys key_next = key_first; key_next != key_end; ++key_next){
      ...
      last1 += l_block;
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first, first1, comp));
   }

   merge_bufferless(is_range1_A ? first1 : last1, first_irr2, last_irr2, comp);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first, last_irr2, comp));
}
   ...
   if(max_collected){
      ...
      if(xbuf.capacity() >= max_collected){
         ...
         while(u != last && h < max_collected){
            typename XBuf::iterator const r = methcla_boost::movelib::lower_bound(ph0, xbuf.end(), *u, comp);
            //If key not found add it to [h, h+h0)
            if(r == xbuf.end() || comp(*u, *r) ){
               RandIt const new_h0 = methcla_boost::move(search_end, u, h0);
               search_end = u;
               ++search_end;
               ++h;
               xbuf.insert(r, u);
         ...
         methcla_boost::move_backward(first, h0, h0+h);
         methcla_boost::move(xbuf.data(), xbuf.end(), first);
      }
      else{
         while(u != last && h < max_collected){
            RandIt const r = methcla_boost::movelib::lower_bound(h0, search_end, *u, comp);
            //If key not found add it to [h, h+h0)
...
{
   methcla_boost::movelib::inplace_stable_sort(first, last, comp);
}
                    ...
                    , XBuf & xbuf)
{
   ...
   ::methcla_boost::movelib::ignore(xbuf);
}
      ...
      , XBuf &xbuf)
{
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first, last, methcla_boost::movelib::unantistable(comp)));
}
                    ...
                    , XBuf & xbuf)
{
   ...
   assert(methcla_boost::movelib::is_sorted_and_unique(first, last, comp));
}
   ...
   , Compare comp, bool const is_stable, Op op)
{
   ...
   for(; n_block_left; --n_block_left){
      ...
      methcla_boost::movelib::ignore(last_min);

      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first_reg, last_reg, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!next_key_idx || methcla_boost::movelib::is_sorted(first_min, last_min, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT((!next_key_idx || !comp(*first_reg, *first_min )));

      OutputIt orig_dest = dest;
      methcla_boost::movelib::ignore(orig_dest);
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(orig_dest, dest, comp));

      if(first_reg == dest){
         dest = next_key_idx ? ::methcla_boost::adl_move_swap_ranges(first_min, last_min, first_reg)
                             : last_reg;
      }
      else{
...
//   assert(n_block_a || n_block_b);
   ...
   for (; n_block_left; --n_block_left) {
      ...
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first1, last1, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first2, last2, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!n_block_left || methcla_boost::movelib::is_sorted(first_min, last_min, comp));
      ...
      if(is_range1_A == is_range2_A){
         ...
         methcla_boost::movelib::ignore(unmerged);
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first-l_block, unmerged, comp));
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first-l_block, buffer, comp));
   ...
   methcla_boost::movelib::ignore(buffer);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first-l_block, buffer, comp));
}
   ...
   , RandItBuf const buf_first)
{
   ...
   methcla_boost::movelib::ignore(len);
   ...
   for (; n_block_left; --n_block_left) {
      ...
      methcla_boost::movelib::ignore(buffer_empty);
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(buffer_empty ? methcla_boost::movelib::is_sorted(first1, last1, comp) : methcla_boost::movelib::is_sorted(buffer, buffer_end, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first2, last2, comp));
      BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(!n_block_left || methcla_boost::movelib::is_sorted(first_min, last_min, comp));
      ...
      if(is_range1_A == is_range2_A){
         ...
         methcla_boost::movelib::ignore(res);
         //swap_and_update_key(key_next, key_range2, key_mid, first2, last2, first_min);
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first_min, last_min, comp));
         first1 = first2;
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first, first1, comp));
      }
      else {
         ...
         first1 = unmerged;
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first, unmerged, comp));
      }
   ...
   RandIt res = op(forward_t(), buffer, buffer_end, first1);
   methcla_boost::movelib::ignore(res);
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(first, res, comp));
   BOOST_MOVE_ADAPTIVE_SORT_PRINT_L2("   merge_blocks_w_fwd: ", len);
   ...
   BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(dest, last_irr2, comp));
      ...
      , Op op)
{
   ...
   for(; l_merged < l_build_buf && l_left_space >= l_merged; l_merged = size_type(l_merged*2u)){
      ...
      while((elements_in_blocks - p0) > 2*l_merged) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT(methcla_boost::movelib::is_sorted(pos-l_merged, pos+l_merged, comp));
      ...
      if((elements_in_blocks-p0) > l_merged) {
         ...
         BOOST_MOVE_ADAPTIVE_SORT_INVARIANT
            (methcla_boost::movelib::is_sorted
