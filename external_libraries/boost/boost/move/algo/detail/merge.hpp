...
class adaptive_xbuf
{
   ...
   void move_assign(RandIt first, size_type n)
   {
      ...
      if(n <= m_size){
         methcla_boost::move(first, first+rand_diff_t(n), m_ptr);
         size_type sz = m_size;
         while(sz-- != n){
            m_ptr[sz].~T();
         }
         ...
         RandRawIt result = methcla_boost::move(first, first+rand_diff_t(m_size), m_ptr);
         methcla_boost::uninitialized_move(first+rand_diff_t(m_size), first+rand_diff_t(n), result);
         m_size = n;
      }
   }
   ...
   void push_back(RandIt first, size_type n)
   {
      ...
      methcla_boost::uninitialized_move(first, first+n, m_ptr+m_size);
      m_size += n;
   }
   ...
   iterator add(RandIt it)
   {
      ...
      ::new(&*p_ret) T(::methcla_boost::move(*it));
      ++m_size;
      return p_ret;
   }
   ...
   void insert(iterator pos, RandIt it)
   {
      if(pos == (m_ptr + m_size)){
         ...
         //m_size updated
         methcla_boost::move_backward(pos, m_ptr+m_size-2, m_ptr+m_size-1);
         *pos = methcla_boost::move(*it);
      }
   }
   ...
   void initialize_until(size_type const sz, T &t)
   {
      ...
      if(m_size < sz){
         BOOST_MOVE_TRY
         {
            ::new((void*)&m_ptr[m_size]) T(::methcla_boost::move(t));
            ++m_size;
            for(; m_size != sz; ++m_size){
               ::new((void*)&m_ptr[m_size]) T(::methcla_boost::move(m_ptr[m_size-1]));
            }
            t = ::methcla_boost::move(m_ptr[m_size-1]);
         }
         BOOST_MOVE_CATCH(...)
         {
            while(m_size)
...
RandIt rotate_gcd(RandIt first, RandIt middle, RandIt last)
{
   ...
   if (middle == ret){
      methcla_boost::adl_move_swap_ranges(first, middle, middle);
   }
   else{
      ...
      , Buf &xbuf)
{
   if(first != middle && middle != last && comp(*middle, middle[-1])){
      ...
      if(len1 <= len2){
         first = methcla_boost::movelib::upper_bound(first, middle, *middle, comp);
         xbuf.move_assign(first, size_type(middle-first));
         op_merge_with_right_placed
            (xbuf.data(), xbuf.end(), first, middle, last, comp, op);
      }
      else{
         last = methcla_boost::movelib::lower_bound(middle, last, middle[-1], comp);
...
void merge_bufferless_ON2(RandIt first, RandIt middle, RandIt last, Compare comp)
{
   if((middle - first) < (last - middle)){
      while(first != middle){
         ...
         middle = methcla_boost::movelib::lower_bound(middle, last, *first, comp);
      ...
      while(middle != last){
         RandIt p = methcla_boost::movelib::upper_bound(first, middle, last[-1], comp);
   ...
   , Compare comp)
{
   ...
   while(1) {
      ...
      if (len1 > len2) {
         len11 = len1 / 2;
         first_cut +=  len11;
         second_cut = methcla_boost::movelib::lower_bound(middle, last, *first_cut, comp);
         len22 = size_type(second_cut - middle);
      }
      else {
         len22 = len2 / 2;
         second_cut += len22;
         first_cut = methcla_boost::movelib::upper_bound(first, middle, *second_cut, comp);
   ...
   , Compare comp, Op op)
{
   ...
   while ( first != last ) {
      if (r_first == r_last) {
         ...
         assert(end == r_last);
         methcla_boost::movelib::ignore(end);
         return;
      }
   ...
   while ( first != last && dest_first != original_r_first ) {
      if (r_first == r_last) {
         for(; dest_first != original_r_first; ++dest_first, ++first){
            ::new((iterator_to_raw_pointer)(dest_first)) value_type(::methcla_boost::move(*first));
            d.incr();
         }
         d.release();
         InputOutIterator end = ::methcla_boost::move(first, last, original_r_first);
         assert(end == r_last);
         methcla_boost::movelib::ignore(end);
         return;
      }
      else if (comp(*r_first, *first)) {
         ::new((iterator_to_raw_pointer)(dest_first)) value_type(::methcla_boost::move(*r_first));
         d.incr();
         ++r_first;
      }
      else {
      ...
      typename iter_size<BidirectionalIterator1>::type buffer_size)
{
   if (len1 > len2 && len2 <= buffer_size)
   {
      ...
      {
         BidirectionalIterator2 buffer_end = methcla_boost::move(middle, last, buffer);
         methcla_boost::move_backward(first, middle, last);
         return methcla_boost::move(buffer, buffer_end, first);
      }
      else
         return first;
   }
   else if (len1 <= buffer_size)
   {
      ...
      {
         BidirectionalIterator2 buffer_end = methcla_boost::move(first, middle, buffer);
         BidirectionalIterator1 ret = methcla_boost::move(middle, last, first);
         methcla_boost::move(buffer, buffer_end, ret);
         return ret;
      }
      else
         return last;
                           ...
                           typename  iter_size<BidirectionalIterator>::type uninitialized_len)
{
   ...
   if(uninitialized_len)
   {
      ...
      ::methcla_boost::movelib::adaptive_xbuf<value_type, RandRawIt> xbuf(uninitialized, uninitialized_len);
