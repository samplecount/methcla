...
namespace methcla_boost {
    ...
    // forward declaration of methcla_boost::basic_string_view from Utility
    template<class Ch, class Tr> class basic_string_view;
}

namespace methcla_boost { namespace detail { namespace lcast {
    ...
    private:
        ...
        bool shl_char(T ch) {
            static_assert(sizeof(T) <= sizeof(CharT),
                "methcla_boost::lexical_cast does not support narrowing of char types."
                "Use methcla_boost::locale instead" );
#ifndef BOOST_LEXICAL_CAST_ASSUME_C_LOCALE
            std::locale loc;
        ...
        inline bool shl_signed(const T n) {
            CharT* tmp_finish = buffer + CharacterBufferSize;
            typedef typename methcla_boost::detail::lcast::make_unsigned<T>::type utype;
        ...
        bool shl_real_type(lcast::exact<float> val, char* begin) {
            ...
            finish = start +
                methcla_boost::core::snprintf(begin, CharacterBufferSize,
                "%.*g", static_cast<int>(methcla_boost::detail::lcast_precision<float>::value), val_as_double);
            return finish > start;
        }
        ...
        bool shl_real_type(lcast::exact<double> val, char* begin) {
            finish = start +
                methcla_boost::core::snprintf(begin, CharacterBufferSize,
                "%.*g", static_cast<int>(methcla_boost::detail::lcast_precision<double>::value), val.payload);
            return finish > start;
        }
        ...
        bool shl_real_type(lcast::exact<long double> val, char* begin) {
            finish = start +
                methcla_boost::core::snprintf(begin, CharacterBufferSize,
                "%.*Lg", static_cast<int>(methcla_boost::detail::lcast_precision<long double>::value), val.payload );
            return finish > start;
        }
#else
        bool shl_real_type(lcast::exact<long double> val, char* begin) {
        ...
        bool shl_real_type(lcast::exact<float> val, wchar_t* begin) {
            const double val_as_double = static_cast<double>(val.payload);
            finish = start + methcla_boost::core::swprintf(
                begin, CharacterBufferSize, L"%.*g",
                static_cast<int>(methcla_boost::detail::lcast_precision<float>::value),
                val_as_double
            );
            return finish > start;
        }
        ...
        bool shl_real_type(lcast::exact<double> val, wchar_t* begin) {
            finish = start + methcla_boost::core::swprintf(
              begin, CharacterBufferSize, L"%.*g",
              static_cast<int>(methcla_boost::detail::lcast_precision<double>::value),
              val.payload
            );
            return finish > start;
        }
        ...
        bool shl_real_type(lcast::exact<long double> val, wchar_t* begin) {
            finish = start + methcla_boost::core::swprintf(
                ...
                static_cast<int>(methcla_boost::detail::lcast_precision<long double>::value),
                val.payload
            );
            return finish > start;
        }
    ...
    public:
        ...
        template<class CharTraits, class Alloc>
        bool stream_in(lcast::exact<methcla_boost::container::basic_string<CharT,CharTraits,Alloc>> x) noexcept {
            start = x.payload.data();
            finish = start + x.payload.length();
            return true;
        }
        ...
        bool stream_in(lcast::exact<methcla_boost::conversion::detail::buffer_view<CharT>> x) noexcept {
        ...
        stream_in(lcast::exact<methcla_boost::iterator_range<C*>> x) noexcept {
            auto buf = methcla_boost::conversion::detail::make_buffer_view(x.payload.begin(), x.payload.end());
        ...
        template <class C>
        typename std::enable_if<methcla_boost::detail::is_character<C>::value, bool>::type
        ...
        template <class Type>
        typename std::enable_if<!std::is_floating_point<Type>::value && methcla_boost::detail::lcast::is_signed<Type>::value && !std::is_enum<Type>::value, bool>::type
                stream_in(lcast::exact<Type> x)                  { return shl_signed(x.payload); }

        template <class Type>
        typename std::enable_if<methcla_boost::detail::lcast::is_unsigned<Type>::value && !std::is_enum<Type>::value, bool>::type
        ...
        enable_if_compatible_char_t<C>
        stream_in(lcast::exact<methcla_boost::array<C, N>> x) noexcept {
        ...
        template <class C, class CharTraits>
        enable_if_compatible_char_t<C>
        stream_in(lcast::exact<methcla_boost::basic_string_view<C, CharTraits>> x) noexcept {
            start = reinterpret_cast<const CharT*>(x.payload.data());
            finish = start + x.payload.size();
            return true;
        }
    ...
    private:
        ...
        bool shl_real(T val) {
            ...
            methcla_boost::detail::lcast_set_precision(out_stream, &val);
            return shl_input_streamable(val);
        }

    public:
        template <class Type>
        typename std::enable_if<methcla_boost::detail::is_character<Type>::value && sizeof(char) == sizeof(Type), bool>::type
    ...
    private:
        ...
        bool shr_signed(Type& output) {
            ...
            CharT const minus = lcast_char_constants<CharT>::minus;
            CharT const plus = lcast_char_constants<CharT>::plus;
            typedef typename methcla_boost::detail::lcast::make_unsigned<Type>::type utype;
            utype out_tmp = 0;
            bool const has_minus = Traits::eq(minus, *start);
        ...
        bool shr_using_base_class(InputStreamable& output)
        {
            static_assert(
                ...
                "methcla_boost::lexical_cast can not convert to pointers"
            );

#if defined(BOOST_NO_STRINGSTREAM) || defined(BOOST_NO_STD_LOCALE)
            static_assert(std::is_same<char, CharT>::value,
                "methcla_boost::lexical_cast can not convert, because your STL library does not "
            ...
            methcla_boost::detail::lcast_set_precision(stream, static_cast<InputStreamable*>(0));
        ...
        inline bool shr_xchar(T& output) noexcept {
            static_assert(sizeof(CharT) == sizeof(T),
                "methcla_boost::lexical_cast does not support narrowing of character types."
                "Use methcla_boost::locale instead" );
    ...
    public:
        ...
        bool stream_out(methcla_boost::ulong_long_type& output)     { return shr_unsigned(output); }
        bool stream_out(methcla_boost::long_long_type& output)      { return shr_signed(output); }
        ...
        bool stream_out(methcla_boost::uint128_type& output)        { return shr_unsigned(output); }
        bool stream_out(methcla_boost::int128_type& output)         { return shr_signed(output); }
#endif
        ...
        template<class CharTraits, class Alloc>
        bool stream_out(methcla_boost::container::basic_string<CharT,CharTraits,Alloc>& str) {
            str.assign(start, finish); return true;
        }
        ...
        template <class C, std::size_t N>
        bool stream_out(methcla_boost::array<C, N>& output) noexcept {
            static_assert(sizeof(C) == sizeof(CharT), "");
            return shr_std_array<N>(output);
        }
...
}}} // namespace methcla_boost::detail::lcast
