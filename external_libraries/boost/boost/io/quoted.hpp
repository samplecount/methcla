    ...
    Char delim)
{
    ...
    if (entry) {
        quoted_state<Char> state = methcla_boost::io::detail::quoted_start(string,
            escape, delim);
        std::basic_streambuf<Char, Traits>& buf = *os.rdbuf();
        std::size_t width = static_cast<std::size_t>(os.width());
        if (width <= state.count) {
            if (!methcla_boost::io::detail::quoted_put(buf, state.string, state.size,
            ...
            if (!methcla_boost::io::detail::quoted_put(buf, state.string, state.size,
                    state.count, escape, delim) ||
                !methcla_boost::io::detail::buffer_fill(buf, os.fill(),
                    width - state.count)) {
                return os;
            }
        } else if (!methcla_boost::io::detail::buffer_fill(buf, os.fill(),
                width - state.count) ||
            !methcla_boost::io::detail::quoted_put(buf, state.string, state.size,
                state.count, escape, delim)) {
            return os;
        }
        os.width(0);
...
{
    return methcla_boost::io::detail::quoted_out(os, proxy.string, proxy.escape,
        proxy.delim);
}
    ...
    const quoted_proxy<std::basic_string<Char, Traits, Alloc>*, Char>& proxy)
{
    ...
    {
        methcla_boost::io::ios_flags_saver ifs(is);
        std::noskipws(is);
        proxy.string->clear();
