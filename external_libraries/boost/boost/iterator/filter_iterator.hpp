...
private:
    ...
    struct storage :
        private methcla_boost::empty_value< Predicate >
    {
        using predicate_base = methcla_boost::empty_value< Predicate >;
        ...
        explicit storage(Iter&& end) :
            predicate_base(methcla_boost::empty_init_t{}), m_end(static_cast< Iterator&& >(end))
        {
        }

        template< typename Pred, typename Iter >
        storage(Pred&& pred, Iter&& end) :
            predicate_base(methcla_boost::empty_init_t{}, static_cast< Pred&& >(pred)), m_end(static_cast< Iter&& >(end))
        {
        }
