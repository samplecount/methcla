...
struct result_multideduce1 {
    ...
    typedef typename methcla_boost::mpl::begin<types>::type begin_it;
    typedef typename methcla_boost::mpl::advance<
        begin_it, methcla_boost::mpl::int_<methcla_boost::mpl::size<types>::type::value - 1>
    >::type                                         last_it;

    template <class It, class Dummy = void> // avoid explicit specialization in class scope
    struct deduce_impl {
        typedef typename methcla_boost::mpl::next<It>::type next_t;
        typedef typename methcla_boost::mpl::deref<It>::type value_t;
        typedef decltype(true ? methcla_boost::declval< Visitor& >()( methcla_boost::declval< copy_cv_ref_t< value_t, Variant > >() )
                              : methcla_boost::declval< typename deduce_impl<next_t>::type >()) type;
    };

    template <class Dummy>
    struct deduce_impl<last_it, Dummy> {
        typedef typename methcla_boost::mpl::deref<last_it>::type value_t;
        typedef decltype(methcla_boost::declval< Visitor& >()( methcla_boost::declval< copy_cv_ref_t< value_t, Variant > >() )) type;
    };
...
inline decltype(auto) apply_visitor(Visitor&& visitor, Visitable&& visitable,
    typename methcla_boost::disable_if<
        methcla_boost::detail::variant::has_result_type<Visitor>,
        bool
    >::type = true)
{
    methcla_boost::detail::variant::result_wrapper1<Visitor, Visitable> cpp14_vis(std::forward<Visitor>(visitor));
