...
extract( methcla_boost::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = methcla_boost::get_error_info<ErrorInfo>( *ex );
...
report_error( execution_exception::error_code ec, methcla_boost::exception const* be, char const* format, va_list* args )
{
...
report_error( execution_exception::error_code ec, methcla_boost::exception const* be, char const* format, ... )
{
    va_list args;
...
{
    return methcla_boost::core::demangle(typeid(t).name());
}
#endif
...
// ************************************************************************** //
// **************    methcla_boost::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************         methcla_boost::detail::signal_action         ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************        methcla_boost::detail::signal_handler         ************** //
// ************************************************************************** //
...
int
execution_monitor::catch_signals( methcla_boost::function<int ()> const& F )
{
...
int
execution_monitor::execute( methcla_boost::function<int ()> const& F )
{
    ...
    // methcla_boost::exception (before std::exception, with extended diagnostic)
    catch( methcla_boost::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
                              "%s", methcla_boost::diagnostic_information(ex).c_str() ); }
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<methcla_boost::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
struct forward {
    explicit    forward( methcla_boost::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    methcla_boost::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( methcla_boost::function<void ()> const& F )
{
...
{
    methcla_boost::ignore_unused(mask);
