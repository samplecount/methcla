...
private:
    struct nil_t {};
    template<typename U> static auto  test( U* ) -> decltype(methcla_boost::declval<U>().size());
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::begin(methcla_boost::declval<U&>())); // does not work with methcla_boost::begin
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::end(methcla_boost::declval<U&>())); // does not work with methcla_boost::end
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
#else
    typedef decltype(methcla_boost::declval<
        typename methcla_boost::add_const<
...
struct bt_iterator_traits< T [N], true > {
    typedef typename methcla_boost::add_const<T>::type T_const;
    typedef typename methcla_boost::add_pointer<T_const>::type const_iterator;
