...
template <typename T, bool is_cstring = is_cstring_impl<typename methcla_boost::decay<T>::type>::value >
...
struct deduce_cstring_transform_impl<T, true> {
    typedef typename methcla_boost::add_const<
        typename methcla_boost::remove_pointer<
            typename methcla_boost::decay<T>::type
        >::type
    >::type U;
    typedef methcla_boost::unit_test::basic_cstring<U> type;
};

template <typename T>
struct deduce_cstring_transform_impl< T, false > {
    typedef typename
        methcla_boost::remove_const<
...
struct deduce_cstring_transform_impl< std::basic_string<T, std::char_traits<T> >, false > {
    typedef methcla_boost::unit_test::basic_cstring<typename methcla_boost::add_const<T>::type> type;
};
...
public:
    using type = stringview_cstring_helper<typename methcla_boost::add_const<T>::type, sv_t>;
};
#endif
...
template<typename T, bool is_cstring = is_cstring<typename methcla_boost::decay<T>::type>::value >
...
template<typename T>
struct is_cstring_comparable< methcla_boost::unit_test::basic_cstring<T>, false > : public mpl::true_ {};
...
struct deduce_cstring_transform {
    typedef typename
        methcla_boost::remove_const<
            typename methcla_boost::remove_reference<T>::type
        >::type U;
    typedef typename ut_detail::deduce_cstring_transform_impl<typename methcla_boost::decay<U>::type>::type type;
};
