   ...
   : public OuterAlloc
{
   ...
   inline scoped_allocator_adaptor_base(BOOST_FWD_REF(OuterA2) outerAlloc, const InnerAllocs &...args)
      : outer_allocator_type(::methcla_boost::forward<OuterA2>(outerAlloc))
      , m_inner(args...)
      {}
   ...
   inline scoped_allocator_adaptor_base(BOOST_RV_REF(scoped_allocator_adaptor_base) other)
      : outer_allocator_type(::methcla_boost::move(other.outer_allocator()))
      , m_inner(::methcla_boost::move(other.inner_allocator()))
      {}
   ...
   {
      outer_allocator_type::operator=(methcla_boost::move(other.outer_allocator()));
      m_inner = ::methcla_boost::move(other.inner_allocator());
      return *this;
   }
   ...
   {
      methcla_boost::adl_move_swap(this->outer_allocator(), r.outer_allocator());
      methcla_boost::adl_move_swap(this->m_inner, r.inner_allocator());
   }
         ...
         <OuterAlloc BOOST_CONTAINER_SCOPEDALLOC_DUMMYTRUE, BOOST_CONTAINER_SCOPEDALLOC_ALLINNER>
{
   ...
   inline scoped_allocator_adaptor(BOOST_RV_REF(scoped_allocator_adaptor) other)
      : base_type(::methcla_boost::move(other.base()))
      {}
   ...
   //!
   //! <b>Effects</b>: initializes the OuterAlloc base class with methcla_boost::forward<OuterA2>(outerAlloc) and inner
   ...
   inline scoped_allocator_adaptor(BOOST_FWD_REF(OuterA2) outerAlloc, const InnerAllocs & ...innerAllocs)
      : base_type(::methcla_boost::forward<OuterA2>(outerAlloc), innerAllocs...)
      {}
   #else // #if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES) || defined(BOOST_CONTAINER_DOXYGEN_INVOKED)
   ...
   inline scoped_allocator_adaptor(BOOST_FWD_REF(OuterA2) outerAlloc BOOST_MOVE_I##N BOOST_MOVE_CREF##N)\
      : base_type(::methcla_boost::forward<OuterA2>(outerAlloc) BOOST_MOVE_I##N BOOST_MOVE_ARG##N)\
      {}\
   //
   ...
   {  return static_cast<scoped_allocator_adaptor&>(base_type::operator=(methcla_boost::move(other.base()))); }
   ...
   inline void construct(T* p, BOOST_FWD_REF(Args)...args)
   {
      dtl::dispatch_uses_allocator
         ...
         , this->inner_allocator(), p, ::methcla_boost::forward<Args>(args)...);
   }
   ...
   inline scoped_allocator_adaptor(internal_type_t, BOOST_FWD_REF(OuterA2) outer, const inner_allocator_type& inner)
      : base_type(internal_type_t(), ::methcla_boost::forward<OuterA2>(outer), inner)
   {}
                      ...
                      ,const scoped_allocator_adaptor<OuterA2, BOOST_CONTAINER_SCOPEDALLOC_ALLINNER>& b)
{
   ...
   #else
   const bool has_zero_inner = methcla_boost::container::dtl::is_same<P0, void>::value;
   #endif
