...
class node_handle
{
   ...
   {  ::new((void*)m_nalloc_storage.data, boost_container_new_t()) nallocator_type(::methcla_boost::move(al));   }

   void destroy_deallocate_node()
   {
      methcla_boost::movelib::to_raw_pointer(m_ptr)->destructor(this->node_alloc());
   ...
   {
      if(m_ptr){
         ::new ((void*)m_nalloc_storage.data, boost_container_new_t()) nallocator_type(::methcla_boost::move(nh.node_alloc()));
      ...
      if(was_nh_non_null){
         if(was_this_non_null){
            ...
            BOOST_IF_CONSTEXPR(nator_traits::propagate_on_container_move_assignment::value){
               this->node_alloc() = ::methcla_boost::move(nh.node_alloc());
            }
         }
         else{
      ...
      BOOST_NOEXCEPT_IF(nator_traits::propagate_on_container_swap::value || nator_traits::is_always_equal::value)
   {
      ...
      if(was_nh_non_null){
         if(was_this_non_null){
            BOOST_IF_CONSTEXPR(nator_traits::propagate_on_container_swap::value){
               ::methcla_boost::adl_move_swap(this->node_alloc(), nh.node_alloc());
            }
         }
         else{
            this->move_construct_alloc(nh.node_alloc());
      ...
      ::methcla_boost::adl_move_swap(m_ptr, nh.m_ptr);
   }
...
struct insert_return_type_base
{
   ...
   insert_return_type_base(BOOST_RV_REF(insert_return_type_base) other)
      : inserted(other.inserted), position(other.position), node(methcla_boost::move(other.node))
   {}

   template<class RelatedIt, class RelatedNode>
   insert_return_type_base(bool insert, RelatedIt it, BOOST_RV_REF(RelatedNode) n)
      : inserted(insert), position(it), node(methcla_boost::move(n))
   {}
   ...
   insert_return_type_base & operator=(BOOST_RV_REF(insert_return_type_base) other)
   {
      ...
      node = methcla_boost::move(other.node);
      return *this;
   }
