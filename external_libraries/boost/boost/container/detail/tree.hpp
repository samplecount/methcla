...
using methcla_boost::intrusive::tree_value_compare;

template<class VoidPointer, methcla_boost::container::tree_type_enum tree_type_value, bool OptimizeSize>
struct intrusive_tree_hook;

template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, methcla_boost::container::red_black_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, methcla_boost::container::avl_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, methcla_boost::container::scapegoat_tree, OptimizeSize>
{
...
template<class VoidPointer, bool OptimizeSize>
struct intrusive_tree_hook<VoidPointer, methcla_boost::container::splay_tree, OptimizeSize>
{
   typedef typename dtl::bi::make_bs_set_base_hook
      < dtl::bi::void_pointer<VoidPointer>
      , dtl::bi::link_mode<dtl::bi::normal_link>
...
{
   typedef pair<typename methcla_boost::move_detail::remove_const<T1>::type, T2> type;
};

template <class T, class VoidPointer, methcla_boost::container::tree_type_enum tree_type_value, bool OptimizeSize>
...
template< class NodeType
        ...
        , class KeyCompare
        , class HookType
        , methcla_boost::container::tree_type_enum tree_type_value>
struct intrusive_tree_dispatch;

template<class NodeType, class KeyOfNode, class KeyCompare, class HookType>
struct intrusive_tree_dispatch
   <NodeType, KeyOfNode, KeyCompare, HookType, methcla_boost::container::red_black_tree>
{
...
template<class NodeType, class KeyOfNode, class KeyCompare, class HookType>
struct intrusive_tree_dispatch
   <NodeType, KeyOfNode, KeyCompare, HookType, methcla_boost::container::avl_tree>
{
...
template<class NodeType, class KeyOfNode, class KeyCompare, class HookType>
struct intrusive_tree_dispatch
   <NodeType, KeyOfNode, KeyCompare, HookType, methcla_boost::container::scapegoat_tree>
{
...
template<class NodeType, class KeyOfNode, class KeyCompare, class HookType>
struct intrusive_tree_dispatch
   <NodeType, KeyOfNode, KeyCompare, HookType, methcla_boost::container::splay_tree>
{
...
template<methcla_boost::container::tree_type_enum tree_type_value>
...
class RecyclingCloner
{
      ...
      (typename dtl::if_c<DoMove, node_t &, const node_t&>::type other) const
   {
      if(node_ptr_type p = m_icont.unlink_leftmost_without_rebalance()){
         ...
         return m_holder.create_node(methcla_boost::move(other.get_real_data()));
      }
   }
...
template<class T1, class T2>
struct tree_key_of_value<methcla_boost::container::dtl::pair<T1, T2>, int>
{
   typedef dtl::select1st<T1> type;
};
      ...
      >
{
   ...
   typedef methcla_boost::container::reverse_iterator
         ...
         )
   {
      ...
      this->allocate_many_and_construct
         ( first, methcla_boost::container::iterator_udistance(first, last)
      ...
      :  AllocHolder(x.value_comp(), a)
   {
      if(this->node_alloc() == x.node_alloc()){
         ...
         this->icont().clone_from
            (methcla_boost::move(x.icont()), typename AllocHolder::move_cloner(*this), Destroyer(this->node_alloc()));
      }
      //AllocHolder clears in case of exception
   }
   ...
   tree& operator=(BOOST_COPY_ASSIGN_REF(tree) x)
   {
      if (BOOST_LIKELY(this != &x)) {
         ...
         //automatically
         Icont other_tree(::methcla_boost::move(this->icont()));
   ...
   {
      NodePtr tmp = AllocHolder::create_node(methcla_boost::forward<MovableConvertible>(v));
   ...
   std::pair<iterator,bool> insert_unique_convertible(BOOST_FWD_REF(MovableConvertible) v)
   {
      ...
      if(ret.second){
         ret.first = this->insert_unique_commit(methcla_boost::forward<MovableConvertible>(v), data);
      }
      return ret;
   }
   ...
   iterator insert_unique_hint_convertible(const_iterator hint, BOOST_FWD_REF(MovableConvertible) v)
   {
      ...
      if(!ret.second)
         return ret.first;
      return this->insert_unique_commit(methcla_boost::forward<MovableConvertible>(v), data);
   }
   ...
   void priv_move_assign(BOOST_RV_REF(tree) x, dtl::bool_<true> /*steal_resources*/)
   {
      ...
      //Obtain resources
      this->icont() = methcla_boost::move(x.icont());
   }
   ...
   {
      NodePtr tmp = AllocHolder::create_node(methcla_boost::forward<KeyConvertible>(key), methcla_boost::forward<M>(obj));
      scoped_node_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
   ...
   {  return this->emplace_unique_node(AllocHolder::create_node(methcla_boost::forward<Args>(args)...));   }

   template <class... Args>
   inline iterator emplace_hint_unique(const_iterator hint, BOOST_FWD_REF(Args)... args)
   {  return this->emplace_hint_unique_node(hint, AllocHolder::create_node(methcla_boost::forward<Args>(args)...));   }

   template <class... Args>
   iterator emplace_equal(BOOST_FWD_REF(Args)... args)
   {
      NodePtr tmp(AllocHolder::create_node(methcla_boost::forward<Args>(args)...));
      scoped_node_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
      ...
      (BOOST_FWD_REF(KeyType) key, BOOST_FWD_REF(Args)... args)
   {
      ...
      if(ret.second){
         ret.first = this->icont().insert_unique_commit
            (*AllocHolder::create_node(try_emplace_t(), methcla_boost::forward<KeyType>(key), methcla_boost::forward<Args>(args)...), data);
      }
      return std::pair<iterator, bool>(iterator(ret.first), ret.second);
   }
   ...
   {\
      ...
      if(ret.second){\
         ret.first = this->icont().insert_unique_commit\
            (*AllocHolder::create_node(try_emplace_t(), methcla_boost::forward<KeyType>(key) BOOST_MOVE_I##N BOOST_MOVE_FWD##N), data);\
      }\
      return std::pair<iterator, bool>(iterator(ret.first), ret.second);\
   }\
   \
   ...
   std::pair<iterator, bool> insert_or_assign(BOOST_FWD_REF(KeyType) key, BOOST_FWD_REF(M) obj)
   {
      ...
      std::pair<iiterator, bool> ret = this->icont().insert_unique_check(k, KeyNodeCompare(key_comp()), data);
      if(ret.second){
         ret.first = this->priv_insert_or_assign_commit(methcla_boost::forward<KeyType>(key), methcla_boost::forward<M>(obj), data);
      }
      else{
         ret.first->get_data().second = methcla_boost::forward<M>(obj);
      }
      return std::pair<iterator, bool>(iterator(ret.first), ret.second);
   }
   ...
   {
      return this->insert_unique_node(this->end(), methcla_boost::move(nh));
   }
   ...
   insert_return_type insert_unique_node(const_iterator hint, BOOST_RV_REF_BEG_IF_CXX11 node_type BOOST_RV_REF_END_IF_CXX11 nh)
   {
      ...
      if(!nh.empty()){
         ...
         if(ret.second){
            ...
            irt.node = methcla_boost::move(nh);
         }
      }
      else{
...
struct has_trivial_destructor_after_move
   < 
      ::methcla_boost::container::dtl::tree
         <T, KeyOfValue, Compare, Allocator, Options>
   >
{
   typedef typename ::methcla_boost::container::dtl::tree<T, KeyOfValue, Compare, Allocator, Options>::allocator_type allocator_type;
   ...
   BOOST_STATIC_CONSTEXPR bool value =
      ...
      ::methcla_boost::has_trivial_destructor_after_move<Compare>::value;
};
