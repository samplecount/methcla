...
#include <boost/container/detail/is_pair.hpp>   //Forward declares methcla_boost::tuples::tuple
...
template <
  ...
  class T9>
struct is_boost_tuple< methcla_boost::tuples::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> >
{
...
struct disable_if_boost_tuple
   : methcla_boost::container::dtl::disable_if< is_boost_tuple<Tuple> >
{};
...
template<>
struct is_tuple_null<methcla_boost::tuples::null_type>
{
   BOOST_STATIC_CONSTEXPR bool value = true;
};
...
struct piecewise_construct_use
{
   ...
   {  (void)&::methcla_boost::container::piecewise_construct;   }
};
...
#endif
{
   ...
   pair(BOOST_RV_REF(pair) p)
      : first(::methcla_boost::move(BOOST_MOVE_TO_LV(p).first)), second(::methcla_boost::move(BOOST_MOVE_TO_LV(p).second))
   {
      BOOST_CONTAINER_STATIC_ASSERT((sizeof(std::pair<T1, T2>) == sizeof(pair<T1, T2>)));
   }
   ...
   pair(BOOST_FWD_REF(U) u, BOOST_FWD_REF(V) v)
      : first(::methcla_boost::forward<U>(u))
      , second(::methcla_boost::forward<V>(v))
   {
      BOOST_CONTAINER_STATIC_ASSERT((sizeof(std::pair<T1, T2>) == sizeof(pair<T1, T2>)));
   }
   ...
   pair(try_emplace_t, BOOST_FWD_REF(KeyType) k, Args && ...args)
      : first(methcla_boost::forward<KeyType>(k)), second(::methcla_boost::forward<Args>(args)...)\
   {
      BOOST_CONTAINER_STATIC_ASSERT((sizeof(std::pair<T1, T2>) == sizeof(pair<T1, T2>)));
   }
   #else
   ...
   pair( try_emplace_t, BOOST_FWD_REF(KeyType) k BOOST_MOVE_I##N BOOST_MOVE_UREF##N )\
      : first(methcla_boost::forward<KeyType>(k)), second(BOOST_MOVE_FWD##N)\
   {\
      BOOST_CONTAINER_STATIC_ASSERT((sizeof(std::pair<T1, T2>) == sizeof(pair<T1, T2>)));\
   }\
   //
   ...
   pair( piecewise_construct_t\
       ...
       , typename dtl::enable_if_c\
         < pair_impl::is_boost_tuple< BoostTuple<BOOST_MOVE_TARG##N  BOOST_MOVE_I##N BOOST_MOVE_REPEAT(BOOST_MOVE_SUB(10,N),::methcla_boost::tuples::null_type)> >::value &&\
   ...
   #  if !defined(BOOST_CONTAINER_NO_CXX11_DELEGATING_CONSTRUCTORS)
      ...
      pair(Tuple<Args1...>& t1, Tuple<Args2...>& t2, index_tuple<Indexes1...>, index_tuple<Indexes2...>)
         : first (::methcla_boost::forward<Args1>(get<Indexes1>(t1))...)
         , second(::methcla_boost::forward<Args2>(get<Indexes2>(t2))...)
      {  (void) t1; (void)t2; }
   ...
   #  else
      ...
      {  return do_build_from_args<T>(::methcla_boost::move(t), typename build_number_seq<sizeof...(Args)>::type());   }
      ...
      {  (void)t; return T(::methcla_boost::forward<Args>(get<Indexes>(t))...);  }

      public:
      template< template<class ...> class Tuple, class... Args1, class... Args2
              , class = typename pair_impl::disable_if_boost_tuple< Tuple<Args1...> >::type>
      pair(piecewise_construct_t, Tuple<Args1...> t1, Tuple<Args2...> t2)
         : first  (build_from_args<first_type> (::methcla_boost::move(t1)))
         , second (build_from_args<second_type>(::methcla_boost::move(t2)))
      {
         BOOST_CONTAINER_STATIC_ASSERT((sizeof(std::pair<T1, T2>) == sizeof(pair<T1, T2>)));
      }
   #  endif   //BOOST_NO_CXX11_VARIADIC_TEMPLATES
   ...
   {
      first  = ::methcla_boost::move(BOOST_MOVE_TO_LV(p).first);
      second = ::methcla_boost::move(BOOST_MOVE_TO_LV(p).second);
      return *this;
   }

   template <class D, class S>
   typename ::methcla_boost::container::dtl::disable_if_or
      < pair &
      , ::methcla_boost::container::dtl::is_same<T1, D>
      , ::methcla_boost::container::dtl::is_same<T2, S>
      >::type
...
//std::pair copy assignment
   ...
   {
      first  = ::methcla_boost::move(p.first);
      second = ::methcla_boost::move(p.second);
      return *this;
   }
   ...
   void swap(pair& p)
   {
      ::methcla_boost::adl_move_swap(this->first, p.first);
      ::methcla_boost::adl_move_swap(this->second, p.second);
   }
};
...
template<class T1, class T2>
struct has_move_emulation_enabled< ::methcla_boost::container::dtl::pair<T1, T2> >
{
   BOOST_STATIC_CONSTEXPR bool value = true;
};
...
template <class T1, class T2>
struct is_class_or_union< ::methcla_boost::container::dtl::pair<T1, T2> >
...
template <class T1, class T2>
struct is_union< ::methcla_boost::container::dtl::pair<T1, T2> >
//This specialization is needed to avoid instantiation of pair in
//is_class, and allow recursive maps.
{
   BOOST_STATIC_CONSTEXPR bool value = false;
...
template <class T1, class T2>
struct is_class< ::methcla_boost::container::dtl::pair<T1, T2> >
//This specialization is needed to avoid instantiation of pair in
//is_class, and allow recursive maps.
{
   BOOST_STATIC_CONSTEXPR bool value = true;
...
struct is_trivially_copy_assignable<methcla_boost::container::dtl::pair<A,B> >
{
...
template<class A, class B>
struct is_trivially_move_assignable<methcla_boost::container::dtl::pair<A,B> >
{
...
template<class A, class B>
struct is_trivially_copy_constructible<methcla_boost::container::dtl::pair<A,B> >
{
...
template<class A, class B>
struct is_trivially_move_constructible<methcla_boost::container::dtl::pair<A,B> >
{
...
template<class A, class B>
struct is_trivially_destructible<methcla_boost::container::dtl::pair<A,B> >
{
...
template<class A, class B>
struct has_trivial_destructor_after_move<methcla_boost::container::dtl::pair<A,B> >
{
