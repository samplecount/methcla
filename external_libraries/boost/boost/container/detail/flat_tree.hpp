...
#define BOOST_INTRUSIVE_HAS_MEMBER_FUNCTION_CALLABLE_WITH_NS_BEG namespace methcla_boost { namespace container { namespace dtl {
   ...
   (SequenceContainer& dest, typename SequenceContainer::iterator it, Compare comp , dtl::true_)
{
   ...
   value_type *const braw = methcla_boost::movelib::to_raw_pointer(dest.data());
   value_type *const iraw = methcla_boost::movelib::iterator_to_raw_pointer(it);
   ...
   methcla_boost::movelib::adaptive_merge
...
{
   methcla_boost::movelib::adaptive_merge(dest.begin(), it, dest.end(), comp);
}
   ...
   (SequenceContainer& dest, typename SequenceContainer::iterator it, Compare comp, dtl::true_)
{
   ...
   //Don't use iterator_to_raw_pointer for end as debug iterators can assert when
   //"operator ->" is used with the end iterator
   value_type* const eraw = methcla_boost::movelib::to_raw_pointer(dest.data()) + dest.size();

   size_type dest_unused_storage_size;
   value_type* const dest_unused_storage_addr =
      unused_storage<SequenceContainer>::get(dest, dest_unused_storage_size);
   methcla_boost::movelib::adaptive_sort
...
inline void flat_tree_container_inplace_sort_ending //is_contiguous_container == false
   (SequenceContainer& dest, typename SequenceContainer::iterator it, Compare comp , dtl::false_)
{
   methcla_boost::movelib::adaptive_sort(it, dest.end(), comp);
}
   ...
   (SequenceContainer& dest, Iterator first, Iterator last, Compare comp, dtl::false_)
{
   if (first != last) {
      ...
      iterator e = methcla_boost::movelib::inplace_set_unique_difference(first_new, dest.end(), dest.begin(), first_new, comp);
   ...
   (SequenceContainer &tseq, BOOST_RV_REF(SequenceContainer) seq, Compare comp)
{
   ...
   value_type * const seq_beg = methcla_boost::movelib::iterator_to_raw_pointer(seq.data());
...
{
   flat_tree_sort_contiguous_to_adopt(tseq, methcla_boost::move(seq), comp);
   tseq = methcla_boost::move(seq);
}
...
{
   methcla_boost::movelib::adaptive_sort(seq.begin(), seq.end(), comp);
   tseq = methcla_boost::move(seq);
}
   ...
   (SequenceContainer &tseq, BOOST_RV_REF(SequenceContainer) seq, Compare comp, dtl::true_)
{
   typedef typename SequenceContainer::value_type value_type;
   value_type * const seq_beg = methcla_boost::movelib::iterator_to_raw_pointer(seq.data());
   methcla_boost::movelib::pdqsort
      ( seq_beg
      , seq_beg + seq.size()
      , comp);
   seq.erase(methcla_boost::movelib::unique
      (seq.begin(), seq.end(), methcla_boost::movelib::negate<Compare>(comp)), seq.cend());
   tseq = methcla_boost::move(seq);
}
...
void flat_tree_adopt_sequence_unique// is_contiguous_container == false
   (SequenceContainer &tseq, BOOST_RV_REF(SequenceContainer) seq, Compare comp, dtl::false_)
{
   methcla_boost::movelib::pdqsort(seq.begin(), seq.end(), comp);
   seq.erase(methcla_boost::movelib::unique
      (seq.begin(), seq.end(), methcla_boost::movelib::negate<Compare>(comp)), seq.cend());
   tseq = methcla_boost::move(seq);
}
...
template < class Value, class AllocatorOrContainer
         , bool = methcla_boost::container::dtl::is_container<AllocatorOrContainer>::value
         >
...
{
   typedef methcla_boost::container::vector<Value, AllocatorOrVoid> type;
};
...
class flat_tree
{
      ...
      : public value_compare
   {
      ...
      inline Data(BOOST_RV_REF(Data) d)
         : value_compare(methcla_boost::move(static_cast<value_compare&>(d))), m_seq(methcla_boost::move(d.m_seq))
      {}
      ...
      inline Data(BOOST_RV_REF(Data) d, const allocator_t &a)
         : value_compare(methcla_boost::move(static_cast<value_compare&>(d))), m_seq(methcla_boost::move(d.m_seq), a)
      {}
      ...
      {
         this->value_compare::operator=(methcla_boost::move(static_cast<value_compare &>(d)));
         m_seq = methcla_boost::move(d.m_seq);
         return *this;
      }
      ...
      void swap(Data &d)
      {
         ...
         methcla_boost::adl_move_swap(mycomp, othercomp);
   ...
   typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT
      (methcla_boost::container::dtl::, container_type
      ,stored_allocator_type, allocator_type)               stored_allocator_type;

   BOOST_STATIC_CONSTEXPR bool has_stored_allocator_type =
      BOOST_INTRUSIVE_HAS_TYPE(methcla_boost::container::dtl::, container_type, stored_allocator_type);
   ...
   inline flat_tree(BOOST_RV_REF(flat_tree) x)
      ...
      :  m_data(methcla_boost::move(x.m_data))
   { }
   ...
   inline flat_tree(BOOST_RV_REF(flat_tree) x, const allocator_type &a)
      :  m_data(methcla_boost::move(x.m_data), a)
   { }
   ...
   {  m_data = methcla_boost::move(x.m_data); return *this;  }
   ...
   std::pair<iterator,bool> insert_unique(BOOST_RV_REF(value_type) val)
   {
      ...
      insert_commit_data data;
      ret.second = this->priv_insert_unique_prepare(KeyOfValue()(val), data);
      ret.first = ret.second ? this->priv_insert_commit(data, methcla_boost::move(val))
                             : this->begin() + (data.position - this->cbegin());
                             //: iterator(vector_iterator_get_ptr(data.position));
      return ret;
   }
   ...
   iterator insert_equal(BOOST_RV_REF(value_type) mval)
   {
      ...
      i = this->m_data.m_seq.insert(i, methcla_boost::move(mval));
      return i;
   }
   ...
   iterator insert_unique(const_iterator hint, BOOST_RV_REF(value_type) val)
   {
      ...
      insert_commit_data data;
      return this->priv_insert_unique_prepare(hint, KeyOfValue()(val), data)
         ? this->priv_insert_commit(data, methcla_boost::move(val))
         : this->begin() + (data.position - this->cbegin());
         //: iterator(vector_iterator_get_ptr(data.position));
   }
   ...
   iterator insert_equal(const_iterator hint, BOOST_RV_REF(value_type) mval)
   {
      ...
      return this->priv_insert_commit(data, methcla_boost::move(mval));
   }
   ...
   void insert_unique_range(InIt first, InIt last)
   {
      ...
      //Step 2: sort them
      methcla_boost::movelib::pdqsort(it, seq.end(), val_cmp);

      //Step 3: only left unique values from the back not already present in the original range
      typename container_type::iterator const e = methcla_boost::movelib::inplace_set_unique_difference
   ...
   {
      const bool value = methcla_boost::container::dtl::
   ...
   std::pair<iterator, bool> emplace_unique(BOOST_FWD_REF(Args)... args)
   {
      ...
      stored_allocator_traits::construct(a, (value_type *)(&v), ::methcla_boost::forward<Args>(args)... );
      value_type *pval = move_detail::launder_cast<value_type *>(&v);
      value_destructor<stored_allocator_type, value_type> d(a, *pval);
      return this->insert_unique(::methcla_boost::move(*pval));
   }
   ...
   iterator emplace_hint_unique(const_iterator hint, BOOST_FWD_REF(Args)... args)
   {
      ...
      value_type *pval = move_detail::launder_cast<value_type *>(&v);
      value_destructor<stored_allocator_type, value_type> d(a, *pval);
      return this->insert_unique(hint, ::methcla_boost::move(*pval));
   }
   ...
   iterator emplace_equal(BOOST_FWD_REF(Args)... args)
   {
      ...
      value_type *pval = move_detail::launder_cast<value_type *>(&v);
      value_destructor<stored_allocator_type, value_type> d(a, *pval);
      return this->insert_equal(::methcla_boost::move(*pval));
   }
   ...
   iterator emplace_hint_equal(const_iterator hint, BOOST_FWD_REF(Args)... args)
   {
      ...
      value_type *pval = move_detail::launder_cast<value_type *>(&v);
      value_destructor<stored_allocator_type, value_type> d(a, *pval);
      return this->insert_equal(hint, ::methcla_boost::move(*pval));
   }
      ...
      (BOOST_FWD_REF(KeyType) key, BOOST_FWD_REF(Args)... args)
   {
      ...
      if(!ret.second){
         ...
         ret.first = this->m_data.m_seq.emplace(data.position, try_emplace_t(), ::methcla_boost::forward<KeyType>(key), ::methcla_boost::forward<Args>(args)...);
      }
      return ret;
   }
   ...
   {\
      ...
      if(!ret.second){\
         ...
         ret.first = this->m_data.m_seq.emplace(data.position, try_emplace_t(), ::methcla_boost::forward<KeyType>(key) BOOST_MOVE_I##N BOOST_MOVE_FWD##N);\
      }\
      return ret;\
   }\
   template <class KeyType BOOST_MOVE_I##N BOOST_MOVE_CLASS##N>\
   ...
   std::pair<iterator, bool> insert_or_assign(BOOST_FWD_REF(KeyType) key, BOOST_FWD_REF(M) obj)
   {
      ...
      if(!ret.second){
         ret.first  = this->nth(size_type(data.position - this->cbegin()));
         ret.first->second = methcla_boost::forward<M>(obj);
      }
      else{
         ret.first = this->m_data.m_seq.emplace(data.position, methcla_boost::forward<KeyType>(key), methcla_boost::forward<M>(obj));
      }
      return ret;
   }
   ...
   {
      this->insert_unique_range( methcla_boost::make_move_iterator(source.begin())
                               , methcla_boost::make_move_iterator(source.end()));
   }

   template<class C2>
   inline void merge_equal(flat_tree<Value, KeyOfValue, C2, AllocatorOrContainer>& source)
   {
      this->insert_equal_range( methcla_boost::make_move_iterator(source.begin())
                              , methcla_boost::make_move_iterator(source.end()));
   }
   ...
   inline void merge_unique(flat_tree& source)
   {
      ...
      (flat_tree_merge_unique)
         ( this->m_data.m_seq
         , methcla_boost::make_move_iterator(source.m_data.m_seq.begin())
         , methcla_boost::make_move_iterator(source.m_data.m_seq.end())
   ...
   {  return methcla_boost::move(m_data.m_seq);   }
   ...
   {
      (flat_tree_adopt_sequence_equal)( m_data.m_seq, methcla_boost::move(seq), this->priv_value_comp()
         , dtl::bool_<is_contiguous_container<container_type>::value>());
   }

   inline void adopt_sequence_unique(BOOST_RV_REF(container_type) seq)
   {
      (flat_tree_adopt_sequence_unique)(m_data.m_seq, methcla_boost::move(seq), this->priv_value_comp()
         , dtl::bool_<is_contiguous_container<container_type>::value>());
   }
   ...
   void adopt_sequence_equal(ordered_range_t, BOOST_RV_REF(container_type) seq)
   {
      ...
      m_data.m_seq = methcla_boost::move(seq);
   }
      ...
      (insert_commit_data &commit_data, BOOST_FWD_REF(Convertible) convertible)
   {
      return this->m_data.m_seq.emplace
         ...
         , methcla_boost::forward<Convertible>(convertible));
   }
...
class Compare, class AllocatorOrContainer>
struct has_trivial_destructor_after_move<methcla_boost::container::dtl::flat_tree<T, KeyOfValue, Compare, AllocatorOrContainer> >
{
   typedef methcla_boost::container::dtl::flat_tree<T, KeyOfValue, Compare, AllocatorOrContainer> flat_tree;
   typedef typename flat_tree::container_type container_type;
   typedef typename flat_tree::key_compare key_compare;
   BOOST_STATIC_CONSTEXPR bool value = ::methcla_boost::has_trivial_destructor_after_move<container_type>::value &&
                             ::methcla_boost::has_trivial_destructor_after_move<key_compare>::value;
};
