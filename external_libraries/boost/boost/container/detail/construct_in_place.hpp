...
{     methcla_boost::container::allocator_traits<Allocator>::construct(a, dest, *source);  }
...
{
   methcla_boost::container::allocator_traits<Allocator>::construct(a, dest);
}
...
{
   methcla_boost::container::allocator_traits<Allocator>::construct(a, dest, default_init);
}
...
{
   assign_in_place_ref(t.first,  ::methcla_boost::move(u.first));
   assign_in_place_ref(t.second, ::methcla_boost::move(u.second));
}
...
BOOST_CONTAINER_FORCEINLINE void assign_in_place(DstIt dest, value_init_construct_iterator<U>)
{
   ...
   *dest = methcla_boost::move(val.get());
}
