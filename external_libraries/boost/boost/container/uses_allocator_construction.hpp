...
T* uninitialized_construct_using_allocator(T* p, BOOST_FWD_REF(AllocArg) alloc_arg, BOOST_FWD_REF(Args)... args)
{
   ...
   methcla_boost::container::dtl::dispatch_uses_allocator
      (atd, methcla_boost::forward<AllocArg>(alloc_arg), p, methcla_boost::forward<Args>(args)...);
   return p;
}
...
{
   return methcla_boost::container::dtl::construct_dispatch_uses_allocator<T>
      (methcla_boost::forward<AllocArg>(alloc_arg), methcla_boost::forward<Args>(args)...);
}
...
#define BOOST_CONTAINER_USES_ALLOCATOR_CONSTRUCTION_CODE(N) \
   ...
   {\
      methcla_boost::container::dtl::allocator_traits_dummy<T> atd;\
      methcla_boost::container::dtl::dispatch_uses_allocator\
         (atd, methcla_boost::forward<AllocArg>(alloc_arg), p BOOST_MOVE_I##N BOOST_MOVE_FWD##N);\
      return p;\
   }\
   \
   ...
   {\
      return methcla_boost::container::dtl::construct_dispatch_uses_allocator<T>\
         (methcla_boost::forward<AllocArg>(alloc_arg) BOOST_MOVE_I##N BOOST_MOVE_FWD##N);\
   }\
//
