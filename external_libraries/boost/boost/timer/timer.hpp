...
namespace timer
{
  ...
  typedef methcla_boost::int_least64_t nanosecond_type;
...
//  cpu_timer  -------------------------------------------------------------------------//
  ...
  public:
    ...
    std::string   format(short places, const std::string& format) const
                        { return ::methcla_boost::timer::format(elapsed(), places, format); }
    std::string   format(short places = default_places) const
                        { return ::methcla_boost::timer::format(elapsed(), places); }
    //  actions
