...
inline methcla_boost::winapi::UINT_ message_cp_win32()
{
#if defined(BOOST_SYSTEM_USE_UTF8)

    return methcla_boost::winapi::CP_UTF8_;

#else

    return methcla_boost::winapi::CP_ACP_;
...
inline char const * system_category_message_win32( int ev, char * buffer, std::size_t len ) noexcept
{
    ...
    methcla_boost::winapi::UINT_ const code_page = message_cp_win32();

    int r = 0;

#if !defined(BOOST_NO_ANSI_APIS)

    if( code_page == methcla_boost::winapi::CP_ACP_ )
    {
        using namespace methcla_boost::winapi;

        DWORD_ retval = methcla_boost::winapi::FormatMessageA(
...

    {
        ...
        DWORD_ retval = methcla_boost::winapi::FormatMessageW(
        ...
        if( retval != 0 )
        {
            r = methcla_boost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, buffer, static_cast<int>( len ), NULL, NULL );
            methcla_boost::winapi::LocalFree( lpMsgBuf );
...
struct local_free
{
    ...
    ~local_free()
    {
        methcla_boost::winapi::LocalFree( p_ );
    }
};
...
inline std::string system_category_message_win32( int ev )
{
    ...
    int r = methcla_boost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, 0, 0, NULL, NULL );
    ...
    r = methcla_boost::winapi::WideCharToMultiByte( code_page, 0, lpMsgBuf, -1, &buffer[0], r, NULL, NULL );
