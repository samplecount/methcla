...
public:
    ...
    template<int I> typename std::tuple_element<I-1, args_type>::type&& operator[] ( methcla_boost::arg<I> ) const
    {
...
public:
    ...
    template<class V> void accept( V & v ) const
    {
        using methcla_boost::visit_each;
        visit_each( v, f_, 0 );
        l_.accept( v );
    }
...
{
    typedef methcla_boost::arg<I> type;
};
...
{
    typedef typename add_value_2< T, methcla_boost::is_placeholder< T >::value >::type type;
};
...
// g++ 4.x (and some 5.x) consider methcla_boost::bind<void>( &X::f )
...
template<class R, class F, class... A>
    _bi::bind_t<R, F, typename _bi::list_av<A...>::type>
    BOOST_BIND( methcla_boost::type<R>, F f, A... a )
{
    typedef typename _bi::list_av<A...>::type list_type;
    return _bi::bind_t<R, F, list_type>( std::move(f), list_type( a... ) );
}
