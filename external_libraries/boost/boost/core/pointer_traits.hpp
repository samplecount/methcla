...
struct ptr_to<T*, T, true> {
    static T* pointer_to(T& v) BOOST_NOEXCEPT {
        return methcla_boost::addressof(v);
    }
};
...
-> decltype(methcla_boost::pointer_traits<T>::to_address(v))
{
    return methcla_boost::pointer_traits<T>::to_address(v);
}
...
{
    return methcla_boost::detail::ptr_address(v.operator->(), 0);
}
...
{
    return methcla_boost::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return methcla_boost::to_address(v.operator->());
}
#endif
