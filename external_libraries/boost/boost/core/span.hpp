...
struct span_capacity {
    static constexpr bool value = E == methcla_boost::dynamic_extent || E == N;
};
...
struct span_is_span<methcla_boost::span<T, E> > {
    static constexpr bool value = true;
};
...
template<class T>
using span_ptr = decltype(methcla_boost::data(std::declval<T&>()));
...
struct span_implicit {
    static constexpr bool value = E == methcla_boost::dynamic_extent ||
        N != methcla_boost::dynamic_extent;
};

template<class T, std::size_t E, class U, std::size_t N>
struct span_copyable {
    static constexpr bool value = (N == methcla_boost::dynamic_extent ||
        span_capacity<E, N>::value) && span_convertible<U, T>::value;
};

template<std::size_t E, std::size_t O>
struct span_sub {
    static constexpr std::size_t value = E == methcla_boost::dynamic_extent ?
        methcla_boost::dynamic_extent : E - O;
};
...
template<class T>
struct span_store<T, methcla_boost::dynamic_extent> {
...
template<class T>
struct span_bytes<T, methcla_boost::dynamic_extent> {
    static constexpr std::size_t value = methcla_boost::dynamic_extent;
};
...
public:
    ...
    constexpr span(R&& r) noexcept(noexcept(methcla_boost::data(r)) &&
        noexcept(r.size()))
        : s_(methcla_boost::data(r), r.size()) { }

    template<class R,
        typename std::enable_if<E != dynamic_extent &&
            detail::span_is_range<R, T>::value, int>::type = 0>
    explicit constexpr span(R&& r) noexcept(noexcept(methcla_boost::data(r)) &&
        noexcept(r.size()))
        : s_(methcla_boost::data(r), r.size()) { }
