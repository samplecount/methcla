...
namespace methcla_boost { namespace heap { namespace detail {
...
template < typename T, typename Cmp, bool constant_time_size, typename StabilityCounterType = methcla_boost::uintmax_t, bool stable = false >
    ...
    size_holder< constant_time_size, size_t >
{
    ...
    internal_type make_node( T const& val )
    {
        if ( counter_ == ( std::numeric_limits< stability_counter_type >::max )() - 1 )
            BOOST_THROW_EXCEPTION( std::runtime_error( "methcla_boost::heap counter overflow" ) );
...
class stable_heap_iterator :
    public methcla_boost::iterator_adaptor< stable_heap_iterator< T, ContainerIterator, Extractor >,
                                    ContainerIterator,
                                    T const,
                                    methcla_boost::random_access_traversal_tag >
{
    typedef methcla_boost::iterator_adaptor< stable_heap_iterator, ContainerIterator, T const, methcla_boost::random_access_traversal_tag >
        super_t;
...
struct make_heap_base
{
    ...
    typedef
        typename parameter::binding< Parspec, tag::stability_counter_type, methcla_boost::uintmax_t >::type stability_counter_type;
...
{
    typedef typename methcla_boost::allocator_size_type< Alloc >::type       size_type;
    typedef typename methcla_boost::allocator_difference_type< Alloc >::type difference_type;
    typedef typename Alloc::value_type&                              reference;
    typedef typename Alloc::value_type const&                        const_reference;
    typedef typename methcla_boost::allocator_pointer< Alloc >::type         pointer;
    typedef typename methcla_boost::allocator_const_pointer< Alloc >::type   const_pointer;
};


}}} // namespace methcla_boost::heap::detail
