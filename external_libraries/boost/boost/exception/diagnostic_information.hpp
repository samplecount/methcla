...
methcla_boost
    ...
    exception_detail
        {
        std::string diagnostic_information_impl( methcla_boost::exception const *, std::exception const *, bool, bool );
        }
    ...
    current_exception_diagnostic_information( bool verbose=true)
        {
        methcla_boost::exception const * be=current_exception_cast<methcla_boost::exception const>();
        ...
        if( be || se )
            ...
            return "Dynamic exception type: "+methcla_boost::core::demangle(p->name());
#endif
        else
...
methcla_boost
    ...
    exception_detail
        ...
        inline
        std::string
        diagnostic_information_impl( methcla_boost::exception const * be, std::exception const * se, bool with_what, bool verbose )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef BOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<methcla_boost::exception const *>(se);
            if( !se )
    ...
    diagnostic_information_what( exception const & e, bool verbose=true ) BOOST_NOEXCEPT_OR_NOTHROW
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce methcla_boost::diagnostic_information_what()";
#ifndef BOOST_NO_EXCEPTIONS
            }
        catch(
        ... )
    ...
    exception_detail
        ...
        void
        serialize_diagnostic_information_to_impl_( methcla_boost::exception const * be, std::exception const * se, Encoder & e )
            {
            if( !be && !se )
                return;
#ifndef BOOST_NO_RTTI
