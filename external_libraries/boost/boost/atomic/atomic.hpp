...
public:
    ...
    static_assert(sizeof(value_type) > 0u, "methcla_boost::atomic<T> requires T to be a complete type");
    static_assert(atomics::detail::is_trivially_copyable< value_type >::value, "methcla_boost::atomic<T> requires T to be a trivially copyable type");
