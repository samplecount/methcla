    ...
    public Base
{
    ...
    static BOOST_FORCEINLINE storage_type wait(storage_type const volatile& storage, storage_type old_val, memory_order order) noexcept
    {
        ...
        {
            methcla_boost::winapi::WaitOnAddress(const_cast< storage_type* >(&storage), &old_val, Size, methcla_boost::winapi::infinite);
...
private:
    ...
    ) noexcept(noexcept(Clock::now()))
    {
        ...
        while (new_val == old_val)
        {
            ...
            methcla_boost::winapi::WaitOnAddress
            (
                const_cast< storage_type* >(&storage),
                &old_val,
                Size,
                msec <= static_cast< std::int64_t >(methcla_boost::winapi::max_non_infinite_wait) ?
                    static_cast< methcla_boost::winapi::DWORD_ >(msec) : methcla_boost::winapi::max_non_infinite_wait
            );
...
public:
    ...
    {
        methcla_boost::winapi::WakeByAddressSingle(const_cast< storage_type* >(&storage));
    }

    static BOOST_FORCEINLINE void notify_all(storage_type volatile& storage) noexcept
    {
        methcla_boost::winapi::WakeByAddressAll(const_cast< storage_type* >(&storage));
    }
};
