...
protected:
   ...
   const ::methcla_boost::regex_traits_wrapper<traits>&  
   ...
   m_has_backrefs(false), m_bad_repeats(0), m_has_recursions(false), m_word_mask(0), m_mask_space(0), m_lower_mask(0), m_upper_mask(0), m_alpha_mask(0)
{
   ...
   m_pdata->m_status = ::methcla_boost::regex_constants::error_ok;
               ...
               if(idx >= hash_value_mask)
               {
                  ...
                  if(idx <= 0)
                  {
                     ...
                     if(0 == this->m_pdata->m_status) // update the error code if not already set
                        this->m_pdata->m_status = methcla_boost::regex_constants::error_bad_pattern;
                     //
                     ...
                     if(0 == (this->flags() & regex_constants::no_except))
                     {
                        ...
                        methcla_boost::regex_error e(message, methcla_boost::regex_constants::error_bad_pattern, 0);
                        e.raise();
                     }
                  }
               }
...
void basic_regex_creator<charT, traits>::create_startmap(re_syntax_base* state, unsigned char* l_map, unsigned int* pnull, unsigned char mask, unsigned recursion_count)
{
   ...
   if (recursion_count > BOOST_REGEX_MAX_RECURSION_DEPTH)
   {
      // Oops error:
      if (0 == this->m_pdata->m_status) // update the error code if not already set
         this->m_pdata->m_status = methcla_boost::regex_constants::error_complexity;
      //
      // clear the expression, we should be empty:
      //
      this->m_pdata->m_expression = 0;
      ...
      if (0 == (this->flags() & regex_constants::no_except))
      {
         ...
         methcla_boost::regex_error e(message, methcla_boost::regex_constants::error_complexity, 0);
         e.raise();
      }
      return;
   }
