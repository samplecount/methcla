...
struct trivial_format_traits
{
   ...
   {
      return ::methcla_boost::BOOST_REGEX_DETAIL_NS::global_lower(c);
   }
   static charT toupper(charT c)
   {
      return ::methcla_boost::BOOST_REGEX_DETAIL_NS::global_upper(c);
   }
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_all(unsigned recursion_count)
{
   ...
   while(m_position != m_end)
   {
      ...
      {
      case '&':
         if(m_flags & ::methcla_boost::regex_constants::format_sed)
         {
            ++m_position;
            put(m_results[0]);
            break;
      ...
      case '\\':
         ...
         break;
      case '(':
         if((m_flags & methcla_boost::regex_constants::format_all) && (recursion_count < BOOST_REGEX_MAX_RECURSION_DEPTH))
         {
            ++m_position;
         ...
         break;
      case ')':
         if(m_flags & methcla_boost::regex_constants::format_all)
         {
            return;
         }
         put(*m_position);
         ...
         break;
      case ':':
         if((m_flags & methcla_boost::regex_constants::format_all) && m_have_conditional)
         {
            return;
         }
         put(*m_position);
...
void basic_regex_formatter<OutputIterator, Results, traits, ForwardIter>::format_escape()
{
   ...
   default:
      ...
      if((m_flags & methcla_boost::regex_constants::format_sed) == 0)
      {
         bool breakout = false;
         switch(*m_position)
         {
      ...
      int v = this->toi(m_position, m_position+len, 10);
      if((v > 0) || ((v == 0) && (m_flags & ::methcla_boost::regex_constants::format_sed)))
      {
         put(m_results[v]);
         break;
      }
...
private:
   ...
   static M m;
   static O out;
   static methcla_boost::regex_constants::match_flag_type flags;
public:
...
struct format_functor3
{
   ...
   OutputIter operator()(const Match& m, OutputIter i, methcla_boost::regex_constants::match_flag_type f)
   {
      return do_unwrap_ref(func)(m, i, f);
   }
   template <class OutputIter, class Traits>
   OutputIter operator()(const Match& m, OutputIter i, methcla_boost::regex_constants::match_flag_type f, const Traits&)
   {
      return (*this)(m, i, f);
   }
private:
...
struct format_functor_c_string
{
   ...
   template <class OutputIter>
   OutputIter operator()(const Match& m, OutputIter i, methcla_boost::regex_constants::match_flag_type f, const Traits& t = Traits())
   {
