...
struct regex_data : public named_subexpressions
{
   ...
   regex_data(const ::std::shared_ptr<
      ::methcla_boost::regex_traits_wrapper<traits> >& t) 
   ...
   regex_data() 
      : m_ptraits(new ::methcla_boost::regex_traits_wrapper<traits>()), m_flags(0), m_status(0), m_expression(0), m_expression_len(0), 
         m_mark_count(0), m_first_state(0), m_restart_type(0), 
      m_startmap{ 0 },
         m_can_be_null(0), m_word_mask(0), m_has_recursions(false), m_disable_match_any(false) {}

   ::std::shared_ptr<
      ::methcla_boost::regex_traits_wrapper<traits>
...
public:
   ...
   const ::methcla_boost::regex_traits_wrapper<traits>& get_traits()const
   {
...
#else
      if(!m_pimpl.get())
         methcla_boost::throw_exception(std::logic_error("Can't access subexpressions in an invalid regex."));
#endif
...
namespace detail
{
   ...
   std::shared_ptr<BOOST_REGEX_DETAIL_NS::basic_regex_implementation<charT, Traits> > create_implemenation(const charT* p1, const charT* p2, F f, std::shared_ptr<methcla_boost::regex_traits_wrapper<Traits> > ptraits)
   {
   ...
   std::shared_ptr<BOOST_REGEX_DETAIL_NS::basic_regex_implementation<char, basic_regex<char>::traits_type> > 
      create_implemenation(const char* p1, const char* p2, basic_regex<char>::flag_type f, std::shared_ptr<methcla_boost::regex_traits_wrapper<basic_regex<char>::traits_type> > ptraits);
   std::shared_ptr<BOOST_REGEX_DETAIL_NS::basic_regex_implementation<wchar_t, basic_regex<wchar_t>::traits_type> > 
      create_implemenation(const wchar_t* p1, const wchar_t* p2, basic_regex<wchar_t>::flag_type f, std::shared_ptr<methcla_boost::regex_traits_wrapper<basic_regex<wchar_t>::traits_type> > ptraits);
#endif
}
...
basic_regex<charT, traits>& basic_regex<charT, traits>::do_assign(const charT* p1,
                        ...
                        flag_type f)
{
   m_pimpl = detail::create_implemenation(p1, p2, f, m_pimpl.get() ? m_pimpl->m_ptraits : std::shared_ptr<methcla_boost::regex_traits_wrapper<traits> >());
   return *this;
}
