...
//!
//! These utilities are provided for those users that want to avoid dependence on std::chrono or methcla_boost::chrono or methcla_boost::date_time
...
class usduration
{
   ...
   //!the passed count
   explicit usduration(methcla_boost::uint64_t microsecs = 0u)
      : m_microsecs(microsecs)
   {}

   //!Returns the stored microsecond
   //!count
   methcla_boost::uint64_t get_microsecs() const
   {  return m_microsecs;  }
   ...
   private:
   methcla_boost::uint64_t m_microsecs;
};
...
class ustime
{
   ...
   explicit ustime(methcla_boost::uint64_t microsecs = 0u)
      : m_microsecs(microsecs)
   {}
...
//!a seconds count
inline usduration usduration_from_seconds(methcla_boost::uint64_t sec)
{  return usduration(sec*uint64_t(1000000u));   }

//!Utility that returns a duration from
//!a milliseconds count
inline usduration usduration_from_milliseconds(methcla_boost::uint64_t millisec)
...
{
   return usduration(static_cast<methcla_boost::uint64_t>(d.total_microseconds()));
}
...
inline usduration duration_to_usduration(const Duration &d, typename enable_if_duration<Duration>::type* = 0)
{
   ...
   return usduration(static_cast<methcla_boost::uint64_t>(double(d.count())*factor));
}
