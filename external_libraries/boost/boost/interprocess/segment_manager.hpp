   ...
   :  private MemoryAlgorithm
{
   ...
   //!Allocates nbytes bytes. Throws methcla_boost::interprocess::bad_alloc
   //!on failure
                           ...
                           size_type &prefer_in_recvd_out_size, T *&reuse)
   {
      ...
      const size_type al = ::methcla_boost::container::dtl::alignment_of<T>::value;
                            ...
                            , size_type &prefer_in_recvd_out_size, void *&reuse, size_type sizeof_object, size_type alignof_object )
   {
      void *ret = MemoryAlgorithm::allocation_command
         (command | methcla_boost::interprocess::nothrow_allocation, limit_size, prefer_in_recvd_out_size, reuse, sizeof_object, alignof_object);
      if(!(command & methcla_boost::interprocess::nothrow_allocation) && !ret)
         throw bad_alloc();
      return ret;
   }
   ...
   :  public segment_manager_base<MemoryAlgorithm>
{
      ...
      ,  m_header(static_cast<segment_manager_base_t*>(get_this_pointer()))
   {
      ...
      const std::size_t void_ptr_alignment = methcla_boost::move_detail::alignment_of<void_pointer>::value; (void)void_ptr_alignment;
      BOOST_ASSERT((0 == (std::size_t)this_addr % methcla_boost::move_detail::alignment_of<segment_manager>::value));
   }
   ...
   struct allocator
   {
      typedef methcla_boost::interprocess::allocator<T, segment_manager> type;
   };
   ...
   struct deleter
   {
      typedef methcla_boost::interprocess::deleter<T, segment_manager> type;
   };
   ...
   {
      typedef typename Proxy::object_type object_type;
      BOOST_CONSTEXPR_OR_CONST std::size_t t_alignment = methcla_boost::move_detail::alignment_of<object_type>::value;
   ...
   void priv_anonymous_destroy(const T *object)
   {
      ...
      BOOST_CONSTEXPR_OR_CONST std::size_t t_alignment =
         methcla_boost::move_detail::alignment_of<T>::value;
      const std::size_t front_space = block_header_t::template front_space_without_header<t_alignment>();
   ...
   {
      methcla_boost::interprocess::allocation_type type = ctrl_data->alloc_type();
   ...
   scoped_lock<rmutex> priv_get_lock(bool use_lock)
   {
      ...
      return scoped_lock<rmutex>(methcla_boost::move(local));
   }
