...
struct enable_if_ptime
   : enable_if_c< BOOST_INTRUSIVE_HAS_TYPE(methcla_boost::interprocess::ipcdetail::, T, time_duration_type) >
{};
...
struct enable_if_ptime_duration
   : enable_if_c< BOOST_INTRUSIVE_HAS_TYPE(methcla_boost::interprocess::ipcdetail::, T, rep_type) >
{};

template<class T>
struct enable_if_time_point
   : enable_if_c< BOOST_INTRUSIVE_HAS_TYPE(methcla_boost::interprocess::ipcdetail::, T, clock) >
{};

template<class T>
struct enable_if_duration
   : enable_if_c< BOOST_INTRUSIVE_HAS_TYPE(methcla_boost::interprocess::ipcdetail::, T, rep) >
{};
...
*/
inline methcla_boost::uint64_t file_time_to_microseconds(const methcla_boost::winapi::FILETIME_ & ft)
{
   // shift is difference between 1970-Jan-01 & 1601-Jan-01
   // in 100-nanosecond units
   const methcla_boost::uint64_t shift = 116444736000000000ULL; // (27111902 << 32) + 3577643008

   // 100-nanos since 1601-Jan-01
   methcla_boost::uint64_t ft_as_integer = (static_cast< methcla_boost::uint64_t >(ft.dwHighDateTime) << 32) | static_cast< methcla_boost::uint64_t >(ft.dwLowDateTime);
...
inline methcla_boost::uint64_t universal_time_u64_us()
{
   #ifdef BOOST_HAS_GETTIMEOFDAY
      timeval tv;
      gettimeofday(&tv, 0); //gettimeofday does not support TZ adjust on Linux.
      methcla_boost::uint64_t micros = methcla_boost::uint64_t(tv.tv_sec)*1000000u;
      micros += (methcla_boost::uint64_t)tv.tv_usec;
   #elif defined(BOOST_HAS_FTIME)
      methcla_boost::winapi::FILETIME_ ft;
      methcla_boost::winapi::GetSystemTimeAsFileTime(&ft);
      methcla_boost::uint64_t micros = file_time_to_microseconds(ft); // it will not wrap, since ft is the current time
...
class microsec_clock<TimeType, typename enable_if_ptime<TimeType>::type>
{
   ...
   static time_point universal_time()
   {
         ...
         methcla_boost::uint32_t sub_sec = static_cast<methcla_boost::uint32_t>(tv.tv_usec);
      #elif defined(BOOST_HAS_FTIME)
         methcla_boost::winapi::FILETIME_ ft;
         methcla_boost::winapi::GetSystemTimeAsFileTime(&ft);
         methcla_boost::uint64_t micros = file_time_to_microseconds(ft); // it will not wrap, since ft is the current time
         ...
         methcla_boost::uint32_t sub_sec = static_cast<methcla_boost::uint32_t>(micros % 1000000UL);
      #else
         #error "Unsupported date-time error: neither gettimeofday nor FILETIME support is detected"
      #endif
...
inline methcla_boost::uint64_t duration_to_milliseconds(const Duration &abs_time, typename enable_if_ptime_duration<Duration>::type* = 0)
{
   return static_cast<methcla_boost::uint64_t>(abs_time.total_milliseconds());
}

template<class Duration>
inline methcla_boost::uint64_t duration_to_milliseconds(const Duration &d, typename enable_if_duration<Duration>::type* = 0)
{
   const double factor = double(Duration::period::num)*1000.0/double(Duration::period::den);
   return static_cast<methcla_boost::uint64_t>(double(d.count())*factor);
}

template<class Duration>
inline methcla_boost::uint64_t duration_to_milliseconds(const Duration&d, typename enable_if_usduration<Duration>::type* = 0)
{
