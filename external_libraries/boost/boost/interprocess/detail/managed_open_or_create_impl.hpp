   ...
   : public managed_open_or_create_impl_device_holder<StoreDevice, DeviceAbstraction>
{
   ...
   static const std::size_t RequiredAlignment =
      MemAlignment ? MemAlignment
                   : methcla_boost::container::dtl::alignment_of< methcla_boost::container::dtl::max_align_t >::value
                   ;

   public:
   static const std::size_t ManagedOpenOrCreateUserOffset =
      ct_rounded_size<sizeof(methcla_boost::uint32_t), RequiredAlignment>::value;
   ...
   {
      managed_open_or_create_impl tmp(methcla_boost::move(moved));
      this->swap(tmp);
      return *this;
   }
       ...
       std::size_t size, const void *addr, ConstructFunc construct_func)
   {
      BOOST_INTERPROCESS_TRY{
         ...
         methcla_boost::uint32_t *patomic_word = 0;  //avoid gcc warning
         patomic_word = static_cast<methcla_boost::uint32_t*>(region.get_address());
         methcla_boost::uint32_t previous = atomic_cas32(patomic_word, InitializingSegment, UninitializedSegment);
      ...
      , bool ronly, bool cow)
   {
      ...
      methcla_boost::uint32_t *patomic_word = static_cast<methcla_boost::uint32_t*>(region.get_address());
      methcla_boost::uint32_t value = atomic_read32(patomic_word);
       ...
       ConstructFunc construct_func)
   {
      ...
      if(StoreDevice){
         this->DevHolder::get_device() = methcla_boost::move(dev);
      }
   }
