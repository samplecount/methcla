...
class mapped_region
{
   ...
   {
      mapped_region tmp(methcla_boost::move(other));
      this->swap(tmp);
      return *this;
   }
   ...
   (offset_t mapping_size, offset_t offset, offset_t page_offset, std::size_t &size)
{
   ...
   if(mapping_size < offset ||
      methcla_boost::uintmax_t(mapping_size - (offset - page_offset)) >
         methcla_boost::uintmax_t(std::size_t(-1))){
   ...
   ,  m_file_or_mapping_hnd(ipcdetail::invalid_file())
{
   mapping_handle_t mhandle = mapping.get_mapping_handle();
   {
      ...
      void *base = winapi::map_view_of_file_ex
                                 ...
                                 map_access,
                                 ::methcla_boost::ulong_long_type(offset - page_offset),
...
{
   ::methcla_boost::adl_move_swap(this->m_base, other.m_base);
   ::methcla_boost::adl_move_swap(this->m_size, other.m_size);
   ::methcla_boost::adl_move_swap(this->m_page_offset, other.m_page_offset);
   ::methcla_boost::adl_move_swap(this->m_mode,  other.m_mode);
   #if defined (BOOST_INTERPROCESS_WINDOWS)
   ::methcla_boost::adl_move_swap(this->m_file_or_mapping_hnd, other.m_file_or_mapping_hnd);
   #else
   ::methcla_boost::adl_move_swap(this->m_is_xsi, other.m_is_xsi);
   #endif
}
