...
#include <boost/interprocess/containers/allocation_type.hpp> //methcla_boost::interprocess::allocation_type
...
#include <boost/container/detail/addressof.hpp> //methcla_boost::container::dtl:addressof
...
{
   static const std::size_t value = methcla_boost::container::dtl::alignment_of<T>::value;
};

template <>
struct alignof_value<void>
{
   static const std::size_t value = methcla_boost::container::dtl::alignment_of<void*>::value;
};
...
struct get_or_create_node_pool_func
{
   ...
   //!Can throw methcla_boost::interprocess::bad_alloc
   void operator()()
   {
      //Find or create the node_pool_t
      mp_node_pool =    mp_segment_manager->template find_or_construct
                        <NodePool>(methcla_boost::interprocess::unique_instance)(mp_segment_manager);
...
struct destroy_if_last_link_func
{
   ...
   void operator()()
   {
      ...
      mp_node_pool->get_segment_manager()->template destroy<NodePool>(methcla_boost::interprocess::unique_instance);
   }
...
class cache_impl
{
   ...
   {
      ::methcla_boost::adl_move_swap(mp_node_pool, other.mp_node_pool);
      ::methcla_boost::adl_move_swap(m_cached_nodes, other.m_cached_nodes);
      ::methcla_boost::adl_move_swap(m_max_cached_nodes, other.m_max_cached_nodes);
   }
};
...
class array_allocation_impl
{
   ...
   {  ::new((void*)ipcdetail::to_raw_pointer(ptr), boost_container_new_t()) value_type(::methcla_boost::forward<P>(p));  }
      ...
      , SegmentManager>
{
   ...
   pointer allocate(size_type count)
   {
      ...
      return pointer(static_cast<value_type*>
         (pool->get_segment_manager()->allocate_aligned(count*sizeof(T), methcla_boost::container::dtl::alignment_of<T>::value)));
   }
         ...
         <cached_allocator_impl<T, NodePool, Version>, T, typename NodePool::segment_manager>
{
   ...
   cached_allocator_impl(BOOST_RV_REF(cached_allocator_impl) other)
      : m_cache(methcla_boost::move(BOOST_MOVE_TO_LV(other).m_cache))
   {}
   ...
   pointer allocate(size_type count)
   {
      ...
      return pointer(static_cast<value_type*>
         (this->get_segment_manager()->allocate_aligned(count*sizeof(T), methcla_boost::container::dtl::alignment_of<T>::value)));
   }
   ...
   {  ::methcla_boost::adl_move_swap(alloc1.m_cache, alloc2.m_cache);   }
   ...
   : public private_node_allocator_t
{
 ...
 public:
   ...
   //!Allocates array of count elements. Can throw methcla_boost::interprocess::bad_alloc
   BOOST_INTERPROCESS_NODISCARD
   void *allocate_node()
   {
      //-----------------------
