...
#endif
template <>
struct has_hidden_guard_digits<methcla_boost::math::concepts::real_concept> : public std::false_type {};
template <>
struct has_hidden_guard_digits<methcla_boost::math::concepts::std_real_concept> : public std::false_type {};
...
template <class T, bool b = methcla_boost::math::tools::detail::has_backend_type<T>::value>
...
T float_next_imp(const T& val, const std::true_type&, const Policy& pol)
{
   ...
   int fpclass = (methcla_boost::math::fpclassify)(val);
...
inline double float_next(const double& val, const Policy& pol)
{
   ...
   if(!(methcla_boost::math::isfinite)(val) && (val > 0))
...
inline typename tools::promote_args<T, U>::type nextafter(const T& val, const U& direction, const Policy& pol)
{
   ...
   return val < direction ? methcla_boost::math::float_next<result_type>(val, pol) : val == direction ? val : methcla_boost::math::float_prior<result_type>(val, pol);
}
...
T float_distance_imp(const T& a, const T& b, const std::true_type&, const Policy& pol)
{
   ...
   if(methcla_boost::math::sign(a) != methcla_boost::math::sign(b))
   ...
   //
   (void)frexp(((methcla_boost::math::fpclassify)(a) == (int)FP_SUBNORMAL) ? tools::min_value<T>() : a, &expon);
   ...
   T mb, x, y, z;
   if(((methcla_boost::math::fpclassify)(a) == (int)FP_SUBNORMAL) || (b - a < tools::min_value<T>()))
   {
      //
...
T float_distance_imp(const T& a, const T& b, const std::false_type&, const Policy& pol)
{
   ...
   // significant bits in the representation:
   //
   expon = 1 + ilogb(((methcla_boost::math::fpclassify)(a) == (int)FP_SUBNORMAL) ? tools::min_value<T>() : a);
...
{
   return methcla_boost::math::float_distance(a, b, policies::policy<>());
}
...
{
   return methcla_boost::math::float_advance(val, distance, policies::policy<>());
}
