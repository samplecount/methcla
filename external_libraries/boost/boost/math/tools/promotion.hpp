...
namespace methcla_boost
{
  namespace math
  {
    namespace tools
    {
      ...
      template <class T>
      struct promote_arg {
         using type = typename methcla_boost::math::conditional<methcla_boost::math::is_integral<T>::value, double, T>::type;
      };


      // Promotes two arguments, neither of which is an integer type using an asymmetric
      // convertibility rule.
      template <class T1, class T2, bool = (methcla_boost::math::is_floating_point<T1>::value && methcla_boost::math::is_floating_point<T2>::value)>
      struct pa2_integral_already_removed {
         using type = typename methcla_boost::math::conditional<
            !methcla_boost::math::is_floating_point<T2>::value && methcla_boost::math::is_convertible<T1, T2>::value, 
            T2, T1>::type;
      };
      ...
      struct pa2_integral_already_removed<T1, T2, true> {
         using type = methcla_boost::math::common_type_t<T1, T2, float>;
      };
      ...
      struct promote_args_permissive<T> {
         using type = typename promote_arg<typename methcla_boost::math::remove_cv<T>::type>::type;
      };
      ...
      struct promote_args {
         ...
         //
         static_assert((0 == methcla_boost::math::is_same<type, long double>::value), "Sorry, but this platform does not have sufficient long double support for the special functions to be reliably implemented.");
#endif
      };
