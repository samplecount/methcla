...
namespace methcla_boost::math::ccmath {
...
inline constexpr Real ldexp(Real arg, int exp) noexcept
{
    ...
    {
        return methcla_boost::math::ccmath::abs(arg) == Real(0) ? arg :
               (methcla_boost::math::ccmath::isinf)(arg) ? arg :
               (methcla_boost::math::ccmath::isnan)(arg) ? arg :
               methcla_boost::math::ccmath::detail::ldexp_impl(arg, exp);
    }
    else
    {
        using std::ldexp;
...
{
    return methcla_boost::math::ccmath::ldexp(static_cast<double>(arg), exp);
}

inline constexpr float ldexpf(float arg, int exp) noexcept
{
    return methcla_boost::math::ccmath::ldexp(arg, exp);
}
