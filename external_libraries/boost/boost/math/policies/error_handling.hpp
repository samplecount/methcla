...
{
   using prec_type = typename methcla_boost::math::policies::precision<T, methcla_boost::math::policies::policy<> >::type;
...
#ifndef BOOST_MATH_NO_RTTI
  replace_all_in_string(function, "%1%", methcla_boost::math::policies::detail::name_of<T>());
#else
...
BOOST_MATH_GPU_ENABLED inline T raise_domain_error(
           ...
           const char* message,
           const T& val,
           const ::methcla_boost::math::policies::domain_error< ::methcla_boost::math::policies::throw_on_error>&)
{
   ...
   // we never get here:
   return methcla_boost::math::numeric_limits<T>::quiet_NaN();
#endif
}
...
BOOST_MATH_GPU_ENABLED constexpr T raise_domain_error(
           ...
           const char* ,
           const T& ,
           const ::methcla_boost::math::policies::domain_error< ::methcla_boost::math::policies::ignore_error>&) BOOST_MATH_NOEXCEPT(T)
{
...
BOOST_MATH_GPU_ENABLED inline T raise_domain_error(
           ...
           const char* ,
           const T& ,
           const ::methcla_boost::math::policies::domain_error< ::methcla_boost::math::policies::errno_on_error>&) BOOST_MATH_NOEXCEPT(T)
{
   errno = EDOM;
   // This may or may not do the right thing, but the user asked for the error
...
BOOST_MATH_GPU_ENABLED inline T raise_domain_error(
           ...
           const char* message,
           const T& val,
           const  ::methcla_boost::math::policies::domain_error< ::methcla_boost::math::policies::user_error>&)
{
...
BOOST_MATH_GPU_ENABLED inline T raise_pole_error(
           ...
           const char* message,
           const T& val,
           const  ::methcla_boost::math::policies::pole_error< ::methcla_boost::math::policies::throw_on_error>&)
{
#ifdef BOOST_MATH_NO_EXCEPTIONS
   static_assert(sizeof(T) == 0, "Error handler called with throw_on_error and BOOST_MATH_NO_EXCEPTIONS set.");
#else
   return methcla_boost::math::policies::detail::raise_domain_error(function, message, val,  ::methcla_boost::math::policies::domain_error< ::methcla_boost::math::policies::throw_on_error>());
#endif
}
...
BOOST_MATH_GPU_ENABLED constexpr T raise_pole_error(
           ...
           const char* message,
           const T& val,
           const  ::methcla_boost::math::policies::pole_error< ::methcla_boost::math::policies::ignore_error>&) BOOST_MATH_NOEXCEPT(T)
{
   return  ::methcla_boost::math::policies::detail::raise_domain_error(function, message, val,  ::methcla_boost::math::policies::domain_error< ::methcla_boost::math::policies::ignore_error>());
}
...
BOOST_MATH_GPU_ENABLED constexpr T raise_pole_error(
           ...
           const char* message,
           const T& val,
           const  ::methcla_boost::math::policies::pole_error< ::methcla_boost::math::policies::errno_on_error>&) BOOST_MATH_NOEXCEPT(T)
{
   return  ::methcla_boost::math::policies::detail::raise_domain_error(function, message, val,  ::methcla_boost::math::policies::domain_error< ::methcla_boost::math::policies::errno_on_error>());
}
...
BOOST_MATH_GPU_ENABLED inline T raise_pole_error(
           ...
           const char* message,
           const T& val,
           const  ::methcla_boost::math::policies::pole_error< ::methcla_boost::math::policies::user_error>&)
{
...
BOOST_MATH_GPU_ENABLED inline T raise_overflow_error(
           const char* function,
           const char* message,
           const  ::methcla_boost::math::policies::overflow_error< ::methcla_boost::math::policies::throw_on_error>&)
{
#ifdef BOOST_MATH_NO_EXCEPTIONS
   static_assert(sizeof(T) == 0, "Error handler called with throw_on_error and BOOST_MATH_NO_EXCEPTIONS set.");
#else
   ...
   return methcla_boost::math::numeric_limits<T>::has_infinity ? methcla_boost::math::numeric_limits<T>::infinity() : methcla_boost::math::tools::max_value<T>();
#endif
}
...
BOOST_MATH_GPU_ENABLED constexpr T raise_overflow_error(
           const char* ,
           const char* ,
           const  ::methcla_boost::math::policies::overflow_error< ::methcla_boost::math::policies::ignore_error>&) BOOST_MATH_NOEXCEPT(T)
{
   // This may or may not do the right thing, but the user asked for the error
   // to be ignored so here we go anyway:
   return methcla_boost::math::numeric_limits<T>::has_infinity ? methcla_boost::math::numeric_limits<T>::infinity() : methcla_boost::math::tools::max_value<T>();
...
BOOST_MATH_GPU_ENABLED inline T raise_overflow_error(
           const char* ,
           const char* ,
           const  ::methcla_boost::math::policies::overflow_error< ::methcla_boost::math::policies::errno_on_error>&) BOOST_MATH_NOEXCEPT(T)
{
   errno = ERANGE;
   // This may or may not do the right thing, but the user asked for the error
   // to be silent so here we go anyway:
...
BOOST_MATH_GPU_ENABLED inline T raise_overflow_error(
           const char* function,
           const char* message,
           const  ::methcla_boost::math::policies::overflow_error< ::methcla_boost::math::policies::user_error>&)
{
   return user_overflow_error(function, message, methcla_boost::math::numeric_limits<T>::infinity());
}
           ...
           const  ::methcla_boost::math::policies::overflow_error< ::methcla_boost::math::policies::user_error>&)
{
   ...
   return user_overflow_error(function, m.c_str(), methcla_boost::math::numeric_limits<T>::infinity());
}
...
BOOST_MATH_GPU_ENABLED inline T raise_underflow_error(
           const char* function,
           const char* message,
           const  ::methcla_boost::math::policies::underflow_error< ::methcla_boost::math::policies::throw_on_error>&)
{
#ifdef BOOST_MATH_NO_EXCEPTIONS
   static_assert(sizeof(T) == 0, "Error handler called with throw_on_error and BOOST_MATH_NO_EXCEPTIONS set.");
#else
...
BOOST_MATH_GPU_ENABLED constexpr T raise_underflow_error(
           const char* ,
           const char* ,
           const  ::methcla_boost::math::policies::underflow_error< ::methcla_boost::math::policies::ignore_error>&) BOOST_MATH_NOEXCEPT(T)
{
   // This may or may not do the right thing, but the user asked for the error
   // to be ignored so here we go anyway:
   return T(0);
...
BOOST_MATH_GPU_ENABLED inline T raise_underflow_error(
           const char* /* function */,
           const char* /* message */,
           const  ::methcla_boost::math::policies::underflow_error< ::methcla_boost::math::policies::errno_on_error>&) BOOST_MATH_NOEXCEPT(T)
{
   errno = ERANGE;
   // This may or may not do the right thing, but the user asked for the error
   // to be silent so here we go anyway:
...
BOOST_MATH_GPU_ENABLED inline T raise_underflow_error(
           const char* function,
           const char* message,
           const  ::methcla_boost::math::policies::underflow_error< ::methcla_boost::math::policies::user_error>&)
{
...
BOOST_MATH_GPU_ENABLED inline T raise_denorm_error(
           ...
           const char* message,
           const T& /* val */,
           const  ::methcla_boost::math::policies::denorm_error< ::methcla_boost::math::policies::throw_on_error>&)
{
#ifdef BOOST_MATH_NO_EXCEPTIONS
   static_assert(sizeof(T) == 0, "Error handler called with throw_on_error and BOOST_MATH_NO_EXCEPTIONS set.");
#else
...
BOOST_MATH_GPU_ENABLED inline constexpr T raise_denorm_error(
           ...
           const char* ,
           const T&  val,
           const  ::methcla_boost::math::policies::denorm_error< ::methcla_boost::math::policies::ignore_error>&) BOOST_MATH_NOEXCEPT(T)
{
   // This may or may not do the right thing, but the user asked for the error
   // to be ignored so here we go anyway:
   return val;
...
BOOST_MATH_GPU_ENABLED inline T raise_denorm_error(
           ...
           const char* ,
           const T& val,
           const  ::methcla_boost::math::policies::denorm_error< ::methcla_boost::math::policies::errno_on_error>&) BOOST_MATH_NOEXCEPT(T)
{
   errno = ERANGE;
   // This may or may not do the right thing, but the user asked for the error
   // to be silent so here we go anyway:
...
BOOST_MATH_GPU_ENABLED inline T raise_denorm_error(
           ...
           const char* message,
           const T& val,
           const  ::methcla_boost::math::policies::denorm_error< ::methcla_boost::math::policies::user_error>&)
{
...
BOOST_MATH_GPU_ENABLED inline T raise_evaluation_error(
           ...
           const char* message,
           const T& val,
           const  ::methcla_boost::math::policies::evaluation_error< ::methcla_boost::math::policies::throw_on_error>&)
{
#ifdef BOOST_MATH_NO_EXCEPTIONS
   static_assert(sizeof(T) == 0, "Error handler called with throw_on_error and BOOST_MATH_NO_EXCEPTIONS set.");
#else
   raise_error<methcla_boost::math::evaluation_error, T>(function, message, val);
   // we never get here:
   return T(0);
#endif
}
...
BOOST_MATH_GPU_ENABLED constexpr T raise_evaluation_error(
           ...
           const char* ,
           const T& val,
           const  ::methcla_boost::math::policies::evaluation_error< ::methcla_boost::math::policies::ignore_error>&) BOOST_MATH_NOEXCEPT(T)
{
   // This may or may not do the right thing, but the user asked for the error
   // to be ignored so here we go anyway:
   return val;
...
BOOST_MATH_GPU_ENABLED inline T raise_evaluation_error(
           ...
           const char* ,
           const T& val,
           const  ::methcla_boost::math::policies::evaluation_error< ::methcla_boost::math::policies::errno_on_error>&) BOOST_MATH_NOEXCEPT(T)
{
   errno = EDOM;
   // This may or may not do the right thing, but the user asked for the error
   // to be silent so here we go anyway:
...
BOOST_MATH_GPU_ENABLED inline T raise_evaluation_error(
           ...
           const char* message,
           const T& val,
           const  ::methcla_boost::math::policies::evaluation_error< ::methcla_boost::math::policies::user_error>&)
{
...
BOOST_MATH_GPU_ENABLED inline TargetType raise_rounding_error(
           ...
           const T& val,
           const TargetType&,
           const  ::methcla_boost::math::policies::rounding_error< ::methcla_boost::math::policies::throw_on_error>&)
{
#ifdef BOOST_MATH_NO_EXCEPTIONS
   static_assert(sizeof(T) == 0, "Error handler called with throw_on_error and BOOST_MATH_NO_EXCEPTIONS set.");
#else
   raise_error<methcla_boost::math::rounding_error, T>(function, message, val);
   // we never get here:
   return TargetType(0);
#endif
}
...
BOOST_MATH_GPU_ENABLED constexpr TargetType raise_rounding_error(
           ...
           const T& val,
           const TargetType&,
           const  ::methcla_boost::math::policies::rounding_error< ::methcla_boost::math::policies::ignore_error>&) BOOST_MATH_NOEXCEPT(T)
{
   // This may or may not do the right thing, but the user asked for the error
   // to be ignored so here we go anyway:
   static_assert(methcla_boost::math::numeric_limits<TargetType>::is_specialized, "The target type must have std::numeric_limits specialized.");
   return  val > 0 ? (methcla_boost::math::numeric_limits<TargetType>::max)() : (methcla_boost::math::numeric_limits<TargetType>::is_integer ? (methcla_boost::math::numeric_limits<TargetType>::min)() : -(methcla_boost::math::numeric_limits<TargetType>::max)());
}
...
BOOST_MATH_GPU_ENABLED inline TargetType raise_rounding_error(
           ...
           const T& val,
           const TargetType&,
           const  ::methcla_boost::math::policies::rounding_error< ::methcla_boost::math::policies::errno_on_error>&) BOOST_MATH_NOEXCEPT(T)
{
   errno = ERANGE;
   // This may or may not do the right thing, but the user asked for the error
   // to be silent so here we go anyway:
...
BOOST_MATH_GPU_ENABLED inline TargetType raise_rounding_error(
           ...
           const T& val,
           const TargetType& t,
           const  ::methcla_boost::math::policies::rounding_error< ::methcla_boost::math::policies::user_error>&)
{
...
BOOST_MATH_GPU_ENABLED inline T raise_indeterminate_result_error(
           ...
           const T& val,
           const R& ,
           const ::methcla_boost::math::policies::indeterminate_result_error< ::methcla_boost::math::policies::throw_on_error>&)
{
#ifdef BOOST_MATH_NO_EXCEPTIONS
   static_assert(sizeof(T) == 0, "Error handler called with throw_on_error and BOOST_MATH_NO_EXCEPTIONS set.");
#else
...
BOOST_MATH_GPU_ENABLED inline constexpr T raise_indeterminate_result_error(
           ...
           const T& ,
           const R& result,
           const ::methcla_boost::math::policies::indeterminate_result_error< ::methcla_boost::math::policies::ignore_error>&) BOOST_MATH_NOEXCEPT(T)
{
   // This may or may not do the right thing, but the user asked for the error
   // to be ignored so here we go anyway:
   return result;
...
BOOST_MATH_GPU_ENABLED inline T raise_indeterminate_result_error(
           ...
           const T& ,
           const R& result,
           const ::methcla_boost::math::policies::indeterminate_result_error< ::methcla_boost::math::policies::errno_on_error>&)
{
   errno = EDOM;
   // This may or may not do the right thing, but the user asked for the error
   // to be silent so here we go anyway:
...
BOOST_MATH_GPU_ENABLED inline T raise_indeterminate_result_error(
           ...
           const T& val,
           const R& ,
           const ::methcla_boost::math::policies::indeterminate_result_error< ::methcla_boost::math::policies::user_error>&)
{
...
BOOST_MATH_GPU_ENABLED BOOST_MATH_FORCEINLINE bool check_overflow(T val, R* result, const char* function, const Policy& pol) noexcept(BOOST_MATH_IS_FLOAT(R) && BOOST_MATH_IS_FLOAT(T) && (Policy::value != throw_on_error) && (Policy::value != user_error))
{
   ...
   {
      methcla_boost::math::policies::detail::raise_overflow_error<R>(function, nullptr, pol);
...
BOOST_MATH_GPU_ENABLED BOOST_MATH_FORCEINLINE bool check_underflow(T val, R* result, const char* function, const Policy& pol) noexcept(BOOST_MATH_IS_FLOAT(R) && BOOST_MATH_IS_FLOAT(T) && (Policy::value != throw_on_error) && (Policy::value != user_error))
{
   ...
   {
      *result = static_cast<R>(methcla_boost::math::policies::detail::raise_underflow_error<R>(function, nullptr, pol));
      return true;
   }
   return false;
}
...
BOOST_MATH_GPU_ENABLED BOOST_MATH_FORCEINLINE bool check_denorm(T val, R* result, const char* function, const Policy& pol) noexcept(BOOST_MATH_IS_FLOAT(R) && BOOST_MATH_IS_FLOAT(T) && (Policy::value != throw_on_error) && (Policy::value != user_error))
{
   ...
   {
      *result = static_cast<R>(methcla_boost::math::policies::detail::raise_denorm_error<R>(function, 0, static_cast<R>(val), pol));
      return true;
   }
   return false;
}
           ...
           const Policy&) BOOST_MATH_NOEXCEPT(T)
{
   // This may or may not do the right thing, but the user asked for the error
   // to be ignored so here we go anyway:
   return methcla_boost::math::numeric_limits<T>::has_infinity ? methcla_boost::math::numeric_limits<T>::infinity() : (methcla_boost::math::numeric_limits<T>::max)();
}
...
template <class R, class Policy, class T>
BOOST_MATH_GPU_ENABLED BOOST_MATH_FORCEINLINE R checked_narrowing_cast(T val, const char* function) noexcept(methcla_boost::math::is_floating_point_v<R> && methcla_boost::math::is_floating_point_v<T>)
{
...
template <class T, class Policy>
BOOST_MATH_GPU_ENABLED inline void check_series_iterations(const char* function, methcla_boost::math::uintmax_t max_iter, const Policy& pol) noexcept(methcla_boost::math::is_floating_point_v<T>)
{
   if(max_iter >= policies::get_max_series_iterations<Policy>())
      raise_evaluation_error<T>(
         function,
...
template <class T, class Policy>
BOOST_MATH_GPU_ENABLED inline void check_root_iterations(const char* function, methcla_boost::math::uintmax_t max_iter, const Policy& pol) noexcept(methcla_boost::math::is_floating_point_v<T>)
{
   if(max_iter >= policies::get_max_root_iterations<Policy>())
      raise_evaluation_error<T>(
         function,
...
namespace methcla_boost { namespace math { namespace detail {
...
//
template <class T>
BOOST_MATH_GPU_ENABLED methcla_boost::math::pair<T, T> pair_from_single(const T& val) BOOST_MATH_NOEXCEPT(T)
{
   return methcla_boost::math::make_pair(val, val);
}

}}} // methcla_boost::math::detail
