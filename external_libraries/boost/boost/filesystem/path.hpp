...
private:
    ...
    friend class methcla_boost::filesystem::path;
    friend class path_reverse_iterator;
    friend struct methcla_boost::filesystem::detail::path_algorithms;
...
//    use methcla_boost::io::quoted() to handle spaces in paths
...
{
    return os << methcla_boost::io::quoted(p.template string< std::basic_string< Char > >(), static_cast< Char >('&'));
}
...
operator>>(std::basic_istream< Char, Traits >& is, path& p)
{
    ...
    is >> methcla_boost::io::quoted(str, static_cast< Char >('&'));
    p = str;
    return is;
}
