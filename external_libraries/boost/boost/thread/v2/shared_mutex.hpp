...
public:
    ...
    template <class Rep, class Period>
        bool try_lock_for(const methcla_boost::chrono::duration<Rep, Period>& rel_time);
    template <class Clock, class Duration>
        bool
        try_lock_until(
                      const methcla_boost::chrono::time_point<Clock, Duration>& abs_time);
    void unlock();
    ...
    template <class Rep, class Period>
        bool
        try_lock_shared_for(const methcla_boost::chrono::duration<Rep, Period>& rel_time);
    template <class Clock, class Duration>
        bool
...
public:
    ...
    template <class Rep, class Period>
        ...
        try_lock_upgrade_for(
                            const methcla_boost::chrono::duration<Rep, Period>& rel_time);
    template <class Clock, class Duration>
        bool
...
namespace methcla_boost {
  namespace thread_v2 {
    ...
    class shared_mutex
    {
      typedef methcla_boost::mutex              mutex_t;
      typedef methcla_boost::condition_variable cond_t;
...
#ifdef BOOST_THREAD_USES_CHRONO
      template <class Rep, class Period>
      bool try_lock_shared_for(const methcla_boost::chrono::duration<Rep, Period>& rel_time)
      {
    ...
    inline shared_mutex::~shared_mutex()
    {
      methcla_boost::lock_guard<mutex_t> _(mut_);
    }
    ...
    {
      methcla_boost::unique_lock<mutex_t> lk(mut_);
      gate1_.wait(lk, methcla_boost::bind(&shared_mutex::no_writer, this));
      state_ |= write_entered_;
      gate2_.wait(lk, methcla_boost::bind(&shared_mutex::no_readers, this));
    }
    ...
    {
      methcla_boost::unique_lock<mutex_t> lk(mut_);
      if (!gate1_.wait_until(lk, abs_time, methcla_boost::bind(
            &shared_mutex::no_writer, this)))
      {
        return false;
      }
      state_ |= write_entered_;
      if (!gate2_.wait_until(lk, abs_time, methcla_boost::bind(
    ...
    {
      methcla_boost::unique_lock<mutex_t> lk(mut_);
      if (!gate1_.timed_wait(lk, abs_or_rel_time, methcla_boost::bind(
            &shared_mutex::no_writer, this)))
      {
        return false;
      }
      state_ |= write_entered_;
      if (!gate2_.timed_wait(lk, abs_or_rel_time, methcla_boost::bind(
            &shared_mutex::no_readers, this)))
      {
        state_ &= ~write_entered_;
        return false;
    ...
    {
      methcla_boost::unique_lock<mutex_t> lk(mut_);
      gate1_.wait(lk, methcla_boost::bind(&shared_mutex::no_writer_no_max_readers, this));
...
namespace methcla_boost {
  namespace thread_v2 {
    ...
    {
      methcla_boost::unique_lock<mutex_t> lk(mut_);
      gate1_.wait(lk, methcla_boost::bind(&upgrade_mutex::no_writer_no_upgrader, this));
      state_ |= write_entered_;
      gate2_.wait(lk, methcla_boost::bind(&upgrade_mutex::no_readers, this));
    }
    ...
    {
      methcla_boost::unique_lock<mutex_t> lk(mut_);
      gate1_.wait(lk, methcla_boost::bind(&upgrade_mutex::no_writer_no_max_readers, this));
      count_t num_readers = (state_ & n_readers_) + 1;
      state_ &= ~n_readers_;
      state_ |= num_readers;
    }
    ...
    {
      methcla_boost::unique_lock<mutex_t> lk(mut_);
      gate1_.wait(lk, methcla_boost::bind(&upgrade_mutex::no_writer_no_upgrader_no_max_readers, this));
      count_t num_readers = (state_ & n_readers_) + 1;
      state_ &= ~n_readers_;
