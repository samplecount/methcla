...
struct alignas( cacheline_bytes ) compiletime_sized_freelist_storage
{
    ...
    T* nodes( void ) const
    {
        ...
        return reinterpret_cast< T* >( methcla_boost::alignment::align_up( data_pointer, cacheline_bytes ) );
    }
...
template < typename T, typename Alloc = std::allocator< T > >
struct runtime_sized_freelist_storage : methcla_boost::alignment::aligned_allocator_adaptor< Alloc, cacheline_bytes >
{
    typedef methcla_boost::alignment::aligned_allocator_adaptor< Alloc, cacheline_bytes > allocator_type;
    T*                                                                            nodes_;
    ...
    {
        if ( count > 65535 )
            methcla_boost::throw_exception( std::runtime_error( "boost.lockfree: freelist size is limited to a maximum of "
                                                        "65535 objects" ) );
