 ...
 *
 *  \b Policies:
 *  - \ref methcla_boost::lockfree::allow_multiple_reads, defaults to
 *    \ref methcla_boost::lockfree::allow_multiple_reads "methcla_boost::lockfree::allow_multiple_reads<false>" \n
...
#ifndef BOOST_DOXYGEN_INVOKED
private:
    using spsc_value_signature = parameter::parameters< methcla_boost::parameter::optional< tag::allow_multiple_reads > >;
...
public:
    ...
    /** Construct a \ref methcla_boost::lockfree::spsc_value "spsc_value"
     *
     *  If configured with \ref methcla_boost::lockfree::allow_multiple_reads "methcla_boost::lockfree::allow_multiple_reads<true>" it
    ...
    /** Construct a \ref methcla_boost::lockfree::spsc_value "spsc_value", initialized to a value
     * */
    ...
    /** Writes `value` to the \ref methcla_boost::lockfree::spsc_value "spsc_value"
     *
     * \pre  only one thread is allowed to write data to the \ref methcla_boost::lockfree::spsc_value "spsc_value"
     * \post object will be written to the \ref methcla_boost::lockfree::spsc_value "spsc_value"
     *
    ...
    /// \copydoc methcla_boost::lockfree::spsc_value::write(T&& value)
    ...
    /** Reads content of the \ref methcla_boost::lockfree::spsc_value "spsc_value"
     *
     * \pre     only one thread is allowed to write data to the \ref methcla_boost::lockfree::spsc_value "spsc_value"
     * \post    if read operation is successful, object will be copied to `ret`.
     * \returns `true`, if the read operation is successful, false if the \ref methcla_boost::lockfree::spsc_value "spsc_value" is
     *          configured with \ref methcla_boost::lockfree::allow_multiple_reads
     *          "methcla_boost::lockfree::allow_multiple_reads<false>" and no value is available for reading
     *
     * \note Thread-safe and wait-free
     * */
    bool read( T& ret )
    ...
    /** Reads content of the \ref methcla_boost::lockfree::spsc_value "spsc_value", returning an optional
     *
     * \pre     only one thread is allowed to write data to the \ref methcla_boost::lockfree::spsc_value "spsc_value"
     * \returns `std::optional` with value if successful, `std::nullopt` if spsc_value is configured with \ref
     *          methcla_boost::lockfree::allow_multiple_reads "methcla_boost::lockfree::allow_multiple_reads<false>" and no value is
