...
class compile_time_sized_ringbuffer : public ringbuffer_base< T >
{
    ...
    typedef
        typename methcla_boost::aligned_storage< max_size * sizeof( T ), methcla_boost::alignment_of< T >::value >::type storage_type;
...
public:
    ...
    template < typename U >
    runtime_sized_ringbuffer( typename methcla_boost::allocator_rebind< Alloc, U >::type const& alloc, size_type max_elements ) :
        Alloc( alloc ),
        max_elements_( max_elements + 1 )
    {
...
typedef parameter::parameters< methcla_boost::parameter::optional< tag::capacity >, methcla_boost::parameter::optional< tag::allocator > >
 ...
 *
 *  - \c methcla_boost::lockfree::allocator<>, defaults to \c methcla_boost::lockfree::allocator<std::allocator<T>> <br>
...
public:
    ...
    explicit spsc_queue( typename methcla_boost::allocator_rebind< allocator, U >::type const& )
    {}
     ...
     */
    template < typename U, typename Enabler = std::enable_if< runtime_sized > >
    spsc_queue( size_type element_count, typename methcla_boost::allocator_rebind< allocator, U >::type const& alloc ) :
    ...
    /// \copydoc methcla_boost::lockfree::spsc_queue::push(const T& t)
    bool push( T&& t )
    {
    ...
    /// \copydoc methcla_boost::lockfree::spsc_queue::front() const
    T& front()
    {
        BOOST_ASSERT( read_available() > 0 );
        return base_type::front();
