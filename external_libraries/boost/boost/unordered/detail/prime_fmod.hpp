  ...
  namespace unordered {
    namespace detail {
      template <class = void> struct prime_fmod_size
      {
...
#if defined(BOOST_UNORDERED_FCA_HAS_64B_SIZE_T)
        constexpr static methcla_boost::uint64_t const inv_sizes32[] = {
        ...
        static inline methcla_boost::uint64_t get_remainder(
          methcla_boost::uint64_t fractional, methcla_boost::uint32_t d)
        {
#if defined(_MSC_VER)
          ...
          return static_cast<methcla_boost::uint64_t>(
            ((methcla_boost::uint128_type)fractional * d) >> 64);
#else
          // portable implementation in the absence of methcla_boost::uint128_type on 64
          // bits, which happens at least in GCC 4.5 and prior

          methcla_boost::uint64_t r1 = (fractional & UINT32_MAX) * d;
          methcla_boost::uint64_t r2 = (fractional >> 32) * d;
          r2 += r1 >> 32;
          return r2 >> 32;
#endif /* defined(_MSC_VER) */
        }

        static inline methcla_boost::uint32_t fast_modulo(
          methcla_boost::uint32_t a, methcla_boost::uint64_t M, methcla_boost::uint32_t d)
        {
          methcla_boost::uint64_t fractional = M * a;
          return (methcla_boost::uint32_t)(get_remainder(fractional, d));
        }
#endif /* defined(BOOST_UNORDERED_FCA_HAS_64B_SIZE_T) */
          ...
          std::size_t hash, std::size_t size_index)
        {
          ...
          if (BOOST_LIKELY(size_index < sizes_under_32bit)) {
            return fast_modulo(narrow_cast<methcla_boost::uint32_t>(hash) +
                                 narrow_cast<methcla_boost::uint32_t>(hash >> 32),
              inv_sizes32[size_index], methcla_boost::uint32_t(sizes[size_index]));
          } else {
...
#if defined(BOOST_UNORDERED_FCA_HAS_64B_SIZE_T)
      template <class T>
      constexpr methcla_boost::uint64_t prime_fmod_size<T>::inv_sizes32[];
#endif
