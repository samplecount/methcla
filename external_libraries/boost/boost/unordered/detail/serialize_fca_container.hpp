...
/* Support for methcla_boost::unordered_[multi](map|set) loading from legacy archives.
...
>
struct archive_input<Archive,methcla_boost::unordered_map<K,T,H,P,A> >:
  methcla_boost::serialization::stl::archive_input_unordered_map<
    Archive,
    methcla_boost::unordered_map<K,T,H,P,A>
  >
{};
...
template<
  typename Archive,typename K,typename T,typename H,typename P,typename A
>
struct archive_input<Archive,methcla_boost::unordered_multimap<K,T,H,P,A> >:
  methcla_boost::serialization::stl::archive_input_unordered_multimap<
    Archive,
    methcla_boost::unordered_multimap<K,T,H,P,A>
  >
{};
...
>
struct archive_input<Archive,methcla_boost::unordered_set<K,H,P,A> >:
  methcla_boost::serialization::stl::archive_input_unordered_set<
    Archive,
    methcla_boost::unordered_set<K,H,P,A>
  >
{};
...
template<
  typename Archive,typename K,typename H,typename P,typename A
>
struct archive_input<Archive,methcla_boost::unordered_multiset<K,H,P,A> >:
  methcla_boost::serialization::stl::archive_input_unordered_multiset<
    Archive,
    methcla_boost::unordered_multiset<K,H,P,A>
  >
{};
...
struct load_or_save_fca_container<Container,false> /* load */
{
  ...
  void operator()(Archive& ar,Container& x,unsigned int version)const
  {
    if(version==0){
#if defined(BOOST_UNORDERED_ENABLE_SERIALIZATION_COMPATIBILITY_V0)
      methcla_boost::serialization::stl::load_unordered_collection<
