...
{
  using group_pointer_traits=methcla_boost::pointer_traits<GroupPtr>;
...
private:
  ...
  unsigned char* pc()const noexcept{return methcla_boost::to_address(pc_);}
  table_element_type* p()const noexcept{return methcla_boost::to_address(p_);}
  ...
  {
    using diff_type=
      typename methcla_boost::pointer_traits<char_pointer>::difference_type;
 ...
 * (methcla_boost::unordered_(flat|node)_(map|set) wrappers complete it as
 * appropriate).
 *
 ...
 * checking is done by methcla_boost::unordered_(flat|node)_(map|set).
 */
...
class table:table_core_impl<TypePolicy,Hash,Pred,Allocator>
{
  ...
  using group_type_pointer=typename methcla_boost::pointer_traits<
    typename methcla_boost::allocator_pointer<Allocator>::type
