...
namespace methcla_boost { namespace optional_detail {
...
namespace methcla_boost {
  ...
  class optional : public optional_detail::optional_tag
  {
    ...
    BOOST_CXX14_CONSTEXPR typename ::std::remove_const<T>::type* dataptr() { return ::methcla_boost::addressof(storage.storage_.value_); }
    constexpr const T* dataptr() const { return ::methcla_boost::addressof(storage.storage_.value_); }
  ...
  public:
    ...
    : storage()
    {
      methcla_boost_optional_detail::construct<value_type>(factory, this->dataptr());
      storage.init_ = true;
    }
    ...
    void swap(optional& rhs)
      noexcept(::std::is_nothrow_move_constructible<T>::value && noexcept(methcla_boost::core::invoke_swap(*rhs, *rhs)))
    {
      if (is_initialized())
      {
        if (rhs.is_initialized())
          methcla_boost::core::invoke_swap(contained_val(), rhs.contained_val());
        else
    ...
    {
      return this->is_initialized() ? this->get() : (methcla_boost::throw_exception(methcla_boost::bad_optional_access()), this->get());
    }
    ...
    BOOST_CXX14_CONSTEXPR reference_type value() &
    {
      if (this->is_initialized())
        ...
        methcla_boost::throw_exception(methcla_boost::bad_optional_access());
    }
  ...
  void swap(optional<T>& lhs, optional<T>& rhs)
    noexcept(::std::is_nothrow_move_constructible<T>::value && noexcept(methcla_boost::core::invoke_swap(*lhs, *rhs)))
  {
    lhs.swap(rhs);
  }
