...
#else
# define BOOST_OPTIONAL_DECAY(T) BOOST_DEDUCED_TYPENAME methcla_boost::decay<T>::type
# define BOOST_OPTIONAL_IS_TAGGED(TAG, U) methcla_boost::is_base_of<TAG, BOOST_OPTIONAL_DECAY(U)>
#endif
...
template <typename U>
struct is_in_place_factory : BOOST_OPTIONAL_IS_TAGGED(methcla_boost::in_place_factory_base, U) {};

template <typename U>
struct is_typed_in_place_factory : BOOST_OPTIONAL_IS_TAGGED(methcla_boost::typed_in_place_factory_base, U) {};
...
}} // namespace methcla_boost::optional_detail
...
/** `optional_value_type`: given type `X`:
    * if `X` is an instance of `methcla_boost::optional`, returns its value_type,
...
template <typename U>
struct optional_value_type< ::methcla_boost::optional<U> >
{
  typedef U type;
};
...
/** This type trait returns the following given the expression `f(ref)`:
     * if the result is a specialization of `methcla_boost::optional`: its value_type,
