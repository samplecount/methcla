...
/// \brief Contains methcla_boost::typeindex::ctti_type_index class that is constexpr if C++14
/// constexpr is supported by compiler.
///
/// methcla_boost::typeindex::ctti_type_index class can be used as a drop-in replacement
/// for std::type_index or as a type index that works at compile time, can provide a
/// type name at compile time.
///
/// It is used as the methcla_boost::typeindex::type_index when `typeid()` is not 
...
inline const detail::ctti_data& ctti_construct() noexcept {
    ...
    return *reinterpret_cast<const detail::ctti_data*>(methcla_boost::typeindex::detail::postprocessed_name<T>());
}
...
/// \code
/// static_assert(
///     methcla_boost::typeindex::ctti_type_index:::type_id<int>().name()
///     == std::string_view{"int"}
/// );
/// \endcode
///
...
public:
    ...
    BOOST_CXX14_CONSTEXPR inline ctti_type_index() noexcept
        : data_(methcla_boost::typeindex::detail::postprocessed_name<void>())
    {}
...
#endif
    return !methcla_boost::typeindex::detail::constexpr_strcmp(left, right);
}
...
#endif
    return methcla_boost::typeindex::detail::constexpr_strcmp(left, right) < 0;
}
...
BOOST_CXX14_CONSTEXPR inline ctti_type_index ctti_type_index::type_id() noexcept {
    ...
    return ctti_type_index(methcla_boost::typeindex::detail::postprocessed_name<no_cvr_t>());
}
...
BOOST_CXX14_CONSTEXPR inline ctti_type_index ctti_type_index::type_id_with_cvr() noexcept {
    return ctti_type_index(methcla_boost::typeindex::detail::postprocessed_name<T>());
}
...
inline std::size_t ctti_type_index::hash_code() const noexcept {
    return methcla_boost::hash_range(raw_name(), raw_name() + get_raw_name_length());
}
