...
/// \brief Contains helper macros and implementation details of methcla_boost::typeindex::ctti_type_index.
...
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip BOOST_TYPE_INDEX_CTTI_USER_DEFINED_PARSING; }
    ...
    // sizeof("const char *__cdecl methcla_boost::detail::ctti<") - 1, sizeof(">::n(void)") - 1
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(40, 10, ""); }
#elif defined(_MSC_VER) && !defined(__clang__) && !defined (BOOST_NO_CXX11_NOEXCEPT)
    // sizeof("const char *__cdecl methcla_boost::detail::ctti<") - 1, sizeof(">::n(void) noexcept") - 1
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(40, 19, ""); }
    ...
    // sizeof("static const char *methcla_boost::detail::ctti<") - 1, sizeof("]") - 1, true, "???????????>::n() [T = int"
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(39, 1, "T = "); }
    ...
    // sizeof("static const char *methcla_boost::detail::ctti<") - 1, sizeof(">::n()") - 1
    // note: checked on 3.0
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(39, 6, ""); }
    ...
    // sizeof("static const char *methcla_boost::detail::ctti<") - 1, sizeof("]") - 1, true, "int>::n() [T = int"
    // note: checked on 3.1, 3.4
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(39, 1, "T = "); }
#elif defined(__EDG__) && !defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static cha methcla_boost::detail::ctti<T>::s() [with I = 40U, T = ") - 1, sizeof("]") - 1
    // note: checked on 4.14
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(58, 1, ""); }
#elif defined(__EDG__) && defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static const char *methcla_boost::detail::ctti<T>::n() [with T = ") - 1, sizeof("]") - 1
    // note: checked on 4.14
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(57, 1, ""); }
    ...
    // sizeof("static constexpr char methcla_boost::detail::ctti<T>::s() [with unsigned int I = 0u; } T = ") - 1, sizeof("]") - 1
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(81, 1, ""); }
#elif defined(__GNUC__) && (__GNUC__ >= 7) && !defined(BOOST_NO_CXX14_CONSTEXPR)
    // sizeof("static constexpr char methcla_boost::detail::ctti<T>::s() [with unsigned int I = 0; } T = ") - 1, sizeof("]") - 1
...
constexpr ctti_skip skip() noexcept { return detail::make_ctti_skip(0, 0, ""); }
    ...
    BOOST_CXX14_CONSTEXPR inline int constexpr_strcmp(const char *v1, const char *v2) noexcept {
        ...
        if (methcla_boost::typeindex::detail::is_constant_string(v1) && methcla_boost::typeindex::detail::is_constant_string(v2))
            return methcla_boost::typeindex::detail::constexpr_strcmp_loop(v1, v2);
...
struct ctti {
    ...
    constexpr static const char* impl(::methcla_boost::typeindex::detail::index_seq<Indexes...> ) noexcept {
        using string_type = ::methcla_boost::typeindex::detail::cstring<s<Indexes>()...>;
        return string_type::data_;
    }

    template <unsigned int D = 0> // `D` means `Dummy`
    constexpr static const char* n() noexcept {
        namespace tid = methcla_boost::typeindex::detail;
    #if defined(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE)
    ...
    #if defined(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE)
        return methcla_boost::typeindex::detail::skip_begining< sizeof(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE) >(BOOST_TYPE_INDEX_FUNCTION_SIGNATURE);
    #elif defined(__FUNCSIG__)
        return methcla_boost::typeindex::detail::skip_begining< sizeof(__FUNCSIG__) >(__FUNCSIG__);
    #elif defined(__PRETTY_FUNCTION__) \
                || defined(__GNUC__) \
                || (defined(__SUNPRO_CC) && (__SUNPRO_CC >= 0x5130)) \
                || (defined(__MWERKS__) && (__MWERKS__ >= 0x3000)) \
                ...
                || defined(__DMC__) \
                || defined(__clang__)
        return methcla_boost::typeindex::detail::skip_begining< sizeof(__PRETTY_FUNCTION__) >(__PRETTY_FUNCTION__);
    #else
        methcla_boost::typeindex::detail::failed_to_get_function_name<T>();
        return "";
    #endif
    }
#endif
...
namespace methcla_boost { namespace typeindex { namespace detail {
    ...
    constexpr const char* make_pretty_name(::methcla_boost::typeindex::detail::index_seq<Indexes...> ) noexcept {
        constexpr const char* name = methcla_boost::detail::ctti<T>::n();
        using string_type = ::methcla_boost::typeindex::detail::cstring<name[Indexes]...>;
        return string_type::data_;
    }
    ...
    constexpr const char* postprocessed_name() noexcept {
        ...
        using idx_seq = typename methcla_boost::typeindex::detail::make_index_seq_impl<0, length>::type;
        return methcla_boost::typeindex::detail::make_pretty_name<T>(idx_seq());
    }
#else
    template <class T>
    constexpr const char* postprocessed_name() noexcept {
        return methcla_boost::detail::ctti<T>::n();
    }
