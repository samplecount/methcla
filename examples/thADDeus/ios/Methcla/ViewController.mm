...
- (void)viewDidLoad
{
    ...
    try
    {
        ...
        //        if (OSStatus const* err =
        //        boost::get_error_info<Methcla::Audio::IO::OSStatusInfo>(e)) {
        //            std::cerr <<
...
- (SynthParams)synthParamsForTouch:(UITouch*)touch
{
    ...
    return {.freq = (1.f - pt.y) * 900.f + 100.f, .amp = pt.x};
}
...
- (void)touchesBegan:(NSSet*)touches withEvent:(UIEvent*)event
{
    ...
    for (UITouch* touch in touches)
    {
        ...
        engine->bundle(Methcla::immediately, [&](Methcla::Request& request) {
            synth = request.synth(METHCLA_PLUGINS_SINE_URI, engine->root(),
                                  {ps.freq, 0.05});
            request.mapOutput(synth, 0, Methcla::AudioBusId(0),
                              Methcla::kBusMappingExternal);
            request.mapOutput(synth, 0, Methcla::AudioBusId(1),
                              Methcla::kBusMappingExternal);
            request.bundle(
                engine->currentTime() + kLatency,
                [&](Methcla::Request& request) { request.activate(synth); });
        });
...
- (void)touchesMoved:(NSSet*)touches withEvent:(UIEvent*)event
{
    ...
    {
        Methcla::SynthId synth(
            [[synths objectForKey:[NSValue valueWithPointer:(const void*)touch]]
                longValue]);
        SynthParams ps = [self synthParamsForTouch:touch];
        engine->set(synth, 0, ps.freq);
        //        std::cout << "Synth " << synth << ": freq=" << ps.freq << "
        //        amp=" << ps.amp << std::endl;
    }
}
...
- (void)touchesEnded:(NSSet*)touches withEvent:(UIEvent*)event
{
    for (UITouch* touch in touches)
    {
        Methcla::SynthId synth(
            [[synths objectForKey:[NSValue valueWithPointer:(const void*)touch]]
                longValue]);
        engine->bundle(engine->currentTime() + kLatency,
                       [&](Methcla::Request& request) { request.free(synth); });
